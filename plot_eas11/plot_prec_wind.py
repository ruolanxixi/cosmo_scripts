...
prec_diff = (prec_ctrl - prec_topo1)/prec_ctrl*100
np.seterr(divide='warn', invalid='warn')
prec_diff[np.isnan(prec_diff)] = 0
...
# axs0.contour(lon, lat, prec_ctrl, cs0.levels, colors='k', linewidths=.1)
...
# axs1.contour(lon, lat, prec_topo1, cs1.levels, colors='k', linewidths=.1)
divnorm=colors.TwoSlopeNorm(vmin=-120., vcenter=0., vmax=80)
cs2 = axs2.contourf(lon, lat, prec_diff, transform=ccrs.PlateCarree(), levels=np.linspace(-120, 80, 51), cmap='RdYlBu', norm=divnorm, extend='both')
# axs2.contour(lon, lat, prec_diff, cs2.levels, colors='k', linewidths=.1)
...
gl = []
for ax in axs:
    ax.set_extent([60, 180, 0, 65], crs=ccrs.PlateCarree())
