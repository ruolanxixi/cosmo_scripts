...
cs0 = axs0.contourf(lon, lat, tmp_ctrl, transform=ccrs.PlateCarree(), levels=np.linspace(-5, 35, 17), cmap='YlOrRd', vmin=-5, vmax=35, extend='both')
# axs0.contour(lon, lat, tmp_ctrl, cs0.levels[::2], colors='k', linewidths=.3)
cs1 = axs1.contourf(lon, lat, tmp_topo1, transform=ccrs.PlateCarree(), levels=np.linspace(-5, 35, 17), cmap='YlOrRd', vmin=-5, vmax=35, extend='both')
# axs1.contour(lon, lat, tmp_topo1, cs1.levels[::2], colors='k', linewidths=.3)
divnorm=colors.TwoSlopeNorm(vmin=-10., vcenter=0., vmax=4)
cs2 = axs2.contourf(lon, lat, tmp_diff, transform=ccrs.PlateCarree(), levels=np.linspace(-10, 4, 15), cmap='RdYlBu_r', norm=divnorm, extend='both')
...
fig.savefig('figure_tmp_2m.png', dpi=300)
