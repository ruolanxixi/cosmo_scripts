...
cs0 = axs0.contourf(lon, lat, tmp_ctrl, transform=ccrs.PlateCarree(), levels=np.linspace(-30, 0, 31), cmap='YlOrRd', vmin=-30, vmax=0, extend='both')
# axs0.contour(lon, lat, tmp_ctrl, cs0.levels[::2], colors='k', linewidths=.3)
cs1 = axs1.contourf(lon, lat, tmp_topo1, transform=ccrs.PlateCarree(), levels=np.linspace(-30, 0, 31), cmap='YlOrRd', vmin=-30, vmax=0, extend='both')
# axs1.contour(lon, lat, tmp_topo1, cs1.levels[::2], colors='k', linewidths=.3)
divnorm=colors.TwoSlopeNorm(vmin=-1., vcenter=0., vmax=1)
cs2 = axs2.contourf(lon, lat, tmp_diff, transform=ccrs.PlateCarree(), levels=np.linspace(-1, 1, 21), cmap='RdYlBu_r', norm=divnorm, extend='both')
...
fig.savefig('figure_tmp_DJF.png', dpi=300)
