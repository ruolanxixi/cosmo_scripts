...
mdpath = "/project/pr133/rxiang/data/cosmo/EAS04_ctrl/szn/TOT_PREC/"
...
erapr = xr.open_dataset(f'{erapath}' + 'era5.mo.2001-2005.JJA.remap.04.nc')['tp'].values[0, :, :]*1000
crupr = xr.open_dataset(f'{crupath}' + 'cru.2001-2005.05.JJA.remap.04.nc')['pre'].values[0, :, :]
...
left, bottom, right, top = 0.05, 0.14, 0.995, 0.95
gs = gridspec.GridSpec(1, 3, left=left, bottom=bottom, right=right, top=top, wspace=0.2, hspace=0.1)
for i in range(3):
...
# cax = fig.add_axes([axs[0].get_position().x0, axs[0].get_position().y0 - 0.3, axs[0].get_position().width, 0.05])
...
cax = fig.add_axes([(axs[0].get_position().x0+axs[0].get_position().x1)/2, axs[1].get_position().y0 - 0.3,
...
rect = plt.Rectangle((0.56, 0.91), width=0.44, height=0.08,
...
fig.suptitle('Validation - Summer Precipitation Validation (JJA)', fontsize=16, fontweight='bold')

xmin, xmax = axs[1].get_xbound()
ymin, ymax = axs[1].get_ybound()
y2x_ratio = (ymax - ymin) / (xmax - xmin) * nrow / ncol + 0.11
...
mdpath = "/project/pr133/rxiang/data/cosmo/EAS04_ctrl/szn/T_2M/"
...
np.seterr(divide='ignore', invalid='ignore')
erat = xr.open_dataset(f'{erapath}' + 'era5.mo.2001-2005.JJA.remap.04.nc')['t2m'].values[0, :, :] - 273.15
aphrot = xr.open_dataset(f'{aphropath}' + 'APHRO.2001-2005.025.JJA.remap.04.nc')['tave'].values[0, :, :]
