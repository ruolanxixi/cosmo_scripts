...
for i in range(2):
    sim = sims[i]
    cs[i, 0] = axs[i, 0].pcolormesh(rlon, rlat, all_pr[i], cmap=cmap1, norm=norm1, shading="auto")
    ct[i, 0] = axs[i, 0].contour(rlon, rlat, all_pr_sms[i], levels=np.linspace(2, 14, 5, endpoint=True), colors='maroon', linewidths=.8)
    clabel = axs[i, 0].clabel(ct[i, 0], [2, 5, 8, 11, 14], inline=True, fontsize=11, use_clabeltext=True)
    for l in clabel:
        l.set_rotation(0)
...
cs[2, 0] = axs[2, 0].pcolormesh(rlon, rlat, all_pr[1] - all_pr[0], cmap=cmap2, clim=(-5, 5), shading="auto")
ct[2, 0] = axs[2, 0].contour(rlon, rlat, all_pr_sms[1] - all_pr_sms[0], levels=[-2, -1, 1, 2], colors='maroon',
                       linewidths=.8)
...
axs[0, 0].set_title("Precipitation", fontweight='bold', pad=7, fontsize=13, loc='left')
