...
class Plot_Cross(UserDict):
    ...
    def add_terrain(self, xyline, level=57):
        # FIXME
        f1 = xr.open_dataset('/project/pr133/rxiang/data/extpar/extpar_cross_ctrl.nc')
        f2 = xr.open_dataset('/project/pr133/rxiang/data/extpar/extpar_cross_ctrl.nc')
    ...
    def add_profile(self, var, varname, w_factor=None, colorbar=True):
        ...
        levels = np.linspace(-5, 15, 21, endpoint=True)
        ...
        ctf = self.ax.contourf(np.arange(vert_cross.shape[1]), vcoord[::-1] / 1000, vert_cross,
                               extend='both', cmap=cmap, levels=levels, norm=norm)
        ...
        cbar.set_ticks([-4, -2, 0, 2, 4, 6, 8, 10, 12, 14])
...
if __name__ == '__main__':
    data_d1 = xr.open_dataset(
        '/project/pr133/rxiang/data/cosmo/EAS04_topo1/szn/DIAB_SUM/2001-2005.DIAB_SUM.JJA.nc')
    data_d2 = xr.open_dataset(
        '/project/pr133/rxiang/data/cosmo/EAS04_ctrl/szn/DIAB_SUM/2001-2005.DIAB_SUM.JJA.nc')
    ...
    # DIAB = (data_d1['DIAB_SUM'].values[0, ::-1, ...] - data_d2['DIAB_SUM'].values[0, ::-1, ...])*1000
    DIAB = data_d2['DIAB_SUM'].values[0, ::-1, ...]
    ...
    # data = Plot_Cross(rlon_start=19.04, rlon_end=-9.08, rlat_start=2.13, rlat_end=2.13, zmax=14)
    data.add_profile(DIAB, DIAB)

    data.save_fig('/project/pr133/rxiang/figure/paper1/results/TENV/diab_ctrl_sm', format='png', dpi=550,
                   transparent=True)
    plt.show()
    # data.save_fig()
