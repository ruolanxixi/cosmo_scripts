...
for s in range(len(sims)):
    sim = sims[s]
    path = f'/project/pr133/rxiang/data/cosmo/EAS04_{sim}/monsoon/ALHFL_S/smr'
    data = Dataset(f'{path}' + '/' + '01-05.ALHFL_S.smr.cpm.nc')
    smr = - data.variables['ALHFL_S'][...]
...
levels1 = np.linspace(0, 200, 21, endpoint=True)
cmap1 = cmc.roma_r
# cmap = cbr_wet(15)
...
for i in range(2):
    ...
    ct[i, 0] = axs[i, 0].contour(rlon, rlat, all_sh_sms[i], levels=np.linspace(0, 200, 6, endpoint=True), colors='k', linewidths=.8)
    clabel = axs[i, 0].clabel(ct[i, 0], levels=np.linspace(0, 200, 6, endpoint=True), inline=True, fontsize=11, use_clabeltext=True)
    for l in clabel:
        l.set_rotation(0)
...
cs[2, 0] = axs[2, 0].pcolormesh(rlon, rlat, all_sh[1] - all_sh[0], cmap=cmap2, clim=(-20, 20), shading="auto")
...
cbar = fig.colorbar(cs[1, 0], cax=cax, orientation='horizontal', extend='both', ticks=np.linspace(0, 200, 6, endpoint=True))
...
cbar = fig.colorbar(cs[2, 0], cax=cax, orientation='horizontal', extend='both', ticks=[-20, -10, 0, 10, 20])
cbar.ax.tick_params(labelsize=13)
cbar.ax.set_xlabel('$W/m^2$', fontsize=13)

axs[0, 0].set_title("Surface latent heat", fontweight='bold', pad=7, fontsize=13, loc='left')
...
fig.savefig(plotpath + 'lh.png', dpi=500)
plt.close(fig)
