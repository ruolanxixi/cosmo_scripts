...
for j in range(1):
    ...
    topo1[2, j] = axs[2, j].contour(lon_, lat_, hsurf_ctrl, levels=[1000, 2000, 3000, 4000],
                                    colors='k',
                                    linewidths=1,
                                    transform=ccrs.PlateCarree())
    clabel = axs[2, j].clabel(topo1[2, j], levels=[1000, 2000, 3000, 4000], inline=True, fontsize=11,
                              use_clabeltext=True)
    for l in clabel:
        l.set_rotation(0)
