...
# panel plot
for i in range(nrow):
    cs[i % 2, i // 2] = axs[i % 2, i // 2].pcolormesh(rlon, rlat, mddata[i], cmap=cmc.roma_r, shading="auto")
    ax = plotcosmo04(axs[i % 2, i // 2])

cmap = custom_div_cmap(27, cmc.vik)
norm = colors.TwoSlopeNorm(vmin=-30., vcenter=0., vmax=60.)
...
cb2 = fig.colorbar(cs[1, 1], cax=cax, orientation='horizontal', ticks=np.append(np.linspace(-30, 0, 7), np.linspace(10, 60, 6)), extend='both')
