...
eradata = xr.open_dataset(f'{erapath}{filename}')['lcc'].values[0, :, :]
...
otdata = xr.open_dataset(f'{cerespath}{filename}')['cldarea_low_mon'].values[0, :, :]/100
...
era11data = xr.open_dataset(f'{erapath}{filename}')['lcc'].values[0, :, :]
filename = f'CERES.{year}.1.remap.11.nc'
ot11data = xr.open_dataset(f'{cerespath}{filename}')['cldarea_low_mon'].values[0, :, :]/100
...
fig.savefig(plotpath + 'clcl.png', dpi=300)
plt.close(fig)
