...
cerespath = "/project/pr133/rxiang/data/obs/cl/CERES/remap/"
...
eradata = xr.open_dataset(f'{erapath}{filename}')['hcc'].values[0, :, :]
...
otdata = xr.open_dataset(f'{cerespath}{filename}')['cldarea_high_mon'].values[0, :, :]/100
...
era11data = xr.open_dataset(f'{erapath}{filename}')['hcc'].values[0, :, :]
filename = f'CERES.{year}.1.remap.11.nc'
ot11data = xr.open_dataset(f'{cerespath}{filename}')['cldarea_high_mon'].values[0, :, :]/100
...
# b1.set_label('$W/m^2$', fontsize=12)
cb1.ax.tick_params(labelsize=12)
cax = colorbar(fig, axs[nrow-1, 1], 2, wspace)  # edit here
cb2 = fig.colorbar(cs[1, 1], cax=cax, orientation='horizontal', extend='both')
# b2.set_label('$W/m^2$', fontsize=12)
...
fig.savefig(plotpath + 'clch.png', dpi=300)
plt.close(fig)
