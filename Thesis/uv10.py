...
for i in ("DJF", "JJA"):
    ds = xr.open_mfdataset(f'{dir}' + f'era5.uv10.1985-2014.ymonmean.nc')
    ds = ds.sel(time=ds['time.season'] == i)
    u = ds['u10'].values[...]
    v = ds['v10'].values[...]
    data[i] = {"u": np.nanmean(u, axis=0),
...
gs = gridspec.GridSpec(1, 2, left=0.06, bottom=0.04, right=0.92,
...
for i in range(ncol):
    ...
    gl.xlocator = mticker.FixedLocator([40, 60, 80, 100, 120, 140, 160, 180, -160])
    gl.ylocator = mticker.FixedLocator([-20, -10, 0, 10, 20, 30, 40, 50, 60, 70, 80])


# --
levels = MaxNLocator(nbins=16).tick_values(0, 8)
cmap = cmc.roma_r
...
for i in range(nrow):
    ...
    cbar = fig.colorbar(cs[i, 1].lines, cax=cax, orientation='vertical', extend='max', ticks=[0, 2, 4, 6, 8])
...
for j in range(ncol):
    label = labels[j]
    axs[0, j].set_title(f'{label}', pad=10, fontsize=16, loc='center')

for i in range(nrow):
    axs[i, 0].text(-0.008, 1, '80°N', ha='right', va='center', transform=axs[i, 0].transAxes, fontsize=14)
    axs[i, 0].text(-0.008, 0.8, '60°N', ha='right', va='center', transform=axs[i, 0].transAxes, fontsize=14)
    axs[i, 0].text(-0.008, 0.6, '40°N', ha='right', va='center', transform=axs[i, 0].transAxes, fontsize=14)
    axs[i, 0].text(-0.008, 0.4, '20°N', ha='right', va='center', transform=axs[i, 0].transAxes, fontsize=14)
    axs[i, 0].text(-0.008, 0.2, '0°N', ha='right', va='center', transform=axs[i, 0].transAxes, fontsize=14)
...
for j in range(ncol):
    ...
    axs[nrow - 1, j].text(0.33, -0.02, '80°E', ha='center', va='top', transform=axs[nrow - 1, j].transAxes,
                          fontsize=14)
    axs[nrow - 1, j].text(0.55, -0.02, '120°E', ha='center', va='top', transform=axs[nrow - 1, j].transAxes,
                          fontsize=14)
    axs[nrow - 1, j].text(0.77, -0.02, '160°E', ha='center', va='top', transform=axs[nrow - 1, j].transAxes,
                          fontsize=14)
    axs[nrow - 1, j].text(0.99, -0.02, '160°W', ha='center', va='top', transform=axs[nrow - 1, j].transAxes,
                          fontsize=14)
