...
def soilSaturation(soiltyp, w_so):
    ...
    cporv = {1: 1.0e-10, 2: 1.0e-10, 3: 0.364, 4: 0.445, 5: 0.455, 6: 0.475, 7: 0.507, 8: 0.863, 9: 1.0e-10,
             10: 1.0e-10}
    cfcap = {1: 1.0e-10, 2: 1.0e-10, 3: 0.196, 4: 0.260, 5: 0.340, 6: 0.370, 7: 0.463, 8: 0.763, 9: 1.0e-10,
             10: 1.0e-10}
...
def plotComparison(case, comp, dataName, show=False, imgsDir=None, zlev=None, slev=None):
    ...
    #
    fig = plt.figure(313, figsize=(len(case) * 10, 7));
    fig.clf()
    if show:
    ...
    if (zlev is not None) and (zlev not in case[0].postproc[dataName].data.altitude.values):
        raise RuntimeError(
            'zlev {0:f} not in range: '.format(zlev) + str(case[0].postproc[dataName].data.altitude.values))
    #
    ...
    for iplot in range(len(case) + 1):
        ...
        if iplot != len(case):
            ...
            # #dataMean = dataTot / (case[iplot].postproc[dataName].data.shape[0] * case[iplot].postproc[
            # dataName].data.shape[1]) #if dataMax > 10*dataMean: #    dataMax = 10*dataMean
            #
            ...
            # ax[iplot].set_title(case[iplot].name + ' - land mean: {0:.2e}'.format(dataTot/case[iplot].data[
            # 'nlandpoints']))
            ax[iplot].set_title(
                case[iplot].name + ' - mean: {0:.2e} / m$^2$'.format(dataTot / case[iplot].data['area']))
...
def plotComparison1D(case, dataName, show=False, imgsDir=None, xlims=None, ylims=None):
    ...
    for icase in range(len(case)):
        #
        plt.plot(case[icase].postproc[dataName].data, case[icase].postproc[dataName].data.altitude,
                 label=case[icase].name)
    #
...
def plotComparisonMatrix(case, comp, dataName, meanType='mean', zlev=None, slev=None, show=False, imgsDir=None):
    ...
    num_plots = len(case)
    fig_dpi = 100
    fig = plt.figure(figsize=((num_plots + 1) * 5, num_plots * 5));
    fig.clf()
    if show:
        plt.show(block=False)
    #
    ...
    if imgsDir is not None:
        # plt.tight_layout()
        if zlev is not None:
            plt.savefig(os.path.join(imgsDir, 'm_' + dataName + '_A{0:06d}.png'.format(int(zlev))), bbox_inches='tight',
                        dpi=fig_dpi)
        elif slev is not None:
            plt.savefig(os.path.join(imgsDir, 'm_' + dataName + '_D{0:06d}.png'.format(int(slev * 1000))),
