...
gs = gridspec.GridSpec(2, 2, left=0.07, bottom=0.1, right=0.99,
                       top=0.985, hspace=0.28, wspace=0.18,
...
lb = [['(a) precipitation', '(b) IVT'], ['(c) 500-hPa GPH', '(d) 850-hPa wind']]
for i in range(nrow):
    for j in range(ncol):
        label = lb[i][j]
        axs[i, j] = fig.add_subplot(gs[i, j], projection=rot_pole_crs)
        axs[i, j] = plotcosmo(axs[i, j])
        t = axs[i, j].text(0, 1.03, f'{label}', ha='left', va='bottom',
...
# --
cax = fig.add_axes([axs[0, 0].get_position().x0, axs[0, 0].get_position().y0-0.07, axs[0, 0].get_position().width, 0.025])
...
# --
cax = fig.add_axes([axs[0, 1].get_position().x0, axs[0, 1].get_position().y0-0.07, axs[0, 1].get_position().width, 0.025])
...
scale = 2000
# --
cs[1, 0] = axs[1, 0].pcolormesh(rlon, rlat, data['diff']['FI'], cmap=cmap, norm=norm, shading="auto")

# --
cax = fig.add_axes([axs[1, 0].get_position().x0, axs[1, 0].get_position().y0-0.07, axs[1, 0].get_position().width, 0.025])
cbar = fig.colorbar(cs[1, 0], cax=cax, orientation='horizontal', extend='both', ticks=[-24, -18, -12, -6, 0, 3, 6, 9, 12])
cbar.ax.tick_params(labelsize=13)
...
# fig.savefig(plotpath + 'poster1.png', dpi=500, transparent=True)
plt.close(fig)
