...
class Simulation():
    ...
    def getAvailData(self):
        ...
        for dirPath, subDirs, fileList in os.walk(self.rootDir, followlinks=True):
            ...
            if (rawNCfiles != []) or (extNCfiles != []):
                ...
                self.dirs[dirName] = OutputData(name=dirName, absPath=dirPath, rawNCfiles=rawNCfiles, extNCfiles=extNCfiles)
    ...
    def load(self, which='ext'):
        ...
        if 'SOILTYP' in self.data:
            ...
            self.data['nlandpoints'] = np.sum( self.data['SOILTYP'].values != 9 ) + np.sum( self.data['SOILTYP'].values != 10 )
    ...
    def buildMatrix(self, case):
        ...
        for row in range(num_cases-1):
            for col in range(row+1,num_cases):
                ...
                if self.comp == 'diff':
                    self.M[row][col] = \
                        PostprocessData( data = case[row].postproc[self.dataName].data - case[col].postproc[self.dataName].data )
