...
for j in range(len(climates)):
    ...
    for i in range(len(var2d_names)):
        ...
        data[climate][var] = {}
        if var == 'siconc':
            ds = xr.open_dataset(f'/project/pr133/rxiang/data/pgw/deltas2/SImon/MPI-ESM1-2-HR/{var}_{climate}.nc', use_cftime=True)
        else:
        ...
        if (var == 'tas' or var == 'ts') and (climate == 'piControl' or climate == 'historical'):
...
lat2 = xr.open_dataset('/project/pr133/rxiang/data/pgw/deltas2/SImon/MPI-ESM1-2-HR/siconc_delta.nc')['j'].values[:]
lon2 = xr.open_dataset('/project/pr133/rxiang/data/pgw/deltas2/SImon/MPI-ESM1-2-HR/siconc_delta.nc')['i'].values[:]
lat_, lon_ = np.meshgrid(lon2, lat2)
...
for ii in range(len(var2d_names)):
    ...
    for jj in range(len(mons)):
        ...
        for i in range(3):
            ...
            if var == 'siconc':
                cs[0, i] = axs[0, i].pcolormesh(lon2, lat2, data[sim][var][mon], cmap=cmap, norm=norm, shading="auto",
