...
# PMIP
data['pmip'] = {}
dir = "/project/pr133/rxiang/data/pmip/var/pr/"
data['pmip']['ctrl'] = np.nanmean(xr.open_dataset(f'{dir}' + 'pr_Amon_PMIP4_piControl.nc')['pr'].values[...], axis=0) * 86400
data['pmip']['lgm'] = np.nanmean(xr.open_dataset(f'{dir}' + 'pr_Amon_PMIP4_lgm.nc')['pr'].values[...], axis=0) * 86400
data['pmip']['change'] = data['pmip']['lgm'] - data['pmip']['ctrl']
lat2 = xr.open_dataset(f'{dir}' + 'pr_Amon_PMIP4_piControl.nc')['lat'].values[...]
lon2 = xr.open_dataset(f'{dir}' + 'pr_Amon_PMIP4_piControl.nc')['lon'].values[...]
...
# add proxy data
axs[0, 2].scatter(lon, lat, c=annp, s=20, linewidths=0.5, cmap=cmap, norm=norm, transform=transform_)
axs[1, 2].scatter(lon, lat, c=annp, s=20, linewidths=0.5, cmap=cmap, norm=norm, transform=transform_)
