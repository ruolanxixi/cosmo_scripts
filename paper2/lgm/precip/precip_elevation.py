...
###############################################################################
# Load and process precerature data
###############################################################################
...
for i in prec_prod.keys():
    ...
    # Load precerature data
    ds = xr.open_dataset(prec_prod[i]["file"])
    prec = ds[prec_prod[i]["var_name"]].values
    ds.close()

    # Convert units of precerature data (if necessary)
    if prec_prod[i]["units_in"] == "mm/day":
        pass
    elif prec_prod[i]["units_in"] == "kg/m2/s":
        prec *= 86400
    else:
    ...
    # Compute spatially integrated precerature for regions
    for j in regions:
        prec_seas = np.empty(12 + 4, dtype=np.float32)
        for k in range(12):
            prec_seas[k] = np.nanmean(prec[k, :, :][region_masks[j]])
        prec_seas[12] = np.nanmean(prec[mask_DJF, :, :].mean(axis=0)
                                       [region_masks[j]])  # dry season
        prec_seas[13] = np.nanmean(prec[mask_MAM, :, :].mean(axis=0)
                                   [region_masks[j]])  # rainy season (MJJAS)
        prec_seas[14] = np.nanmean(prec[mask_JJA, :, :].mean(axis=0)
                                   [region_masks[j]])  # dry season
        prec_seas[15] = np.nanmean(prec[mask_SON, :, :].mean(axis=0)
                                   [region_masks[j]])  # dry season

        # -> use grid cell area as weights for more accurate spatial averaging!
        prec_prod_reg[i][j] = prec_seas

    # Compute precerature lapse rates
    prec_grad_MAM = np.empty(num_bin, dtype=np.float32)  # rainy season
    prec_grad_MAM.fill(np.nan)
    prec_grad_JJA = np.empty(num_bin, dtype=np.float32)  # dry season
    prec_grad_JJA.fill(np.nan)
    prec_grad_SON = np.empty(num_bin, dtype=np.float32)  # rainy season
    prec_grad_SON.fill(np.nan)
    prec_grad_DJF = np.empty(num_bin, dtype=np.float32)  # dry season
    prec_grad_DJF.fill(np.nan)
    for j in range(num_bin):
        mask = (topo[i] >= elev_bin_edges[j]) \
               & (topo[i] < elev_bin_edges[j + 1]) & region_masks["HM"]
        if mask.sum() >= 15:
            prec_grad_DJF[j] = np.nanmean(prec[mask_DJF, :, :].mean(axis=0)
                                          [mask])
            prec_grad_MAM[j] = np.nanmean(prec[mask_MAM, :, :].mean(axis=0)
                                         [mask])
            prec_grad_JJA[j] = np.nanmean(prec[mask_JJA, :, :].mean(axis=0)
                                         [mask])
            prec_grad_SON[j] = np.nanmean(prec[mask_SON, :, :].mean(axis=0)
                                          [mask])
    prec_prod_elev[i] = {"DJF": prec_grad_DJF,
                         "MAM": prec_grad_MAM,
                         "JJA": prec_grad_JJA,
                         "SON": prec_grad_SON
                         }
...
gs = gridspec.GridSpec(3, 8, left=0.043, bottom=0.08, right=0.99, top=0.95,
                       hspace=0.05, wspace=0.0,
                       width_ratios=[0.6, 0.12, 0.05, 0.04,
                                     0.22, 0.22, 0.22, 0.22], height_ratios=[1, 0.15, 1])
# -----------------------------------------------------------------------------
...
for i in prec_prod.keys():
    plt.plot(months_num, prec_prod_reg[i]["HM"][:12], color=cols[i],
             zorder=3)
    plt.scatter(months_num, prec_prod_reg[i]["HM"][:12], s=s_small, marker=markers[i],
                color=cols[i], label=i, zorder=3)
# plt.fill_between(x=[4.5, 9.5], y1=-11.0, y2=22.0, color="black", alpha=0.1)
plt.xticks(months_num, months_char, fontsize=14)
# plt.text(x=6.0, y=-1.8, s="Rainy season", fontsize=14)
plt.ylabel("precipitation [mm $day^{-1}$]", labelpad=5, fontsize=14)
plt.yticks(np.arange(1, 11, 1), np.arange(1, 11, 1), fontsize=14)
plt.ylim([0, 11])
plt.axis([0.7, 12.3, 0, 11])
plt.title("(c)", fontsize=14, fontweight="normal", y=1.01,
          loc="left")
plt.legend(loc="upper left", frameon=False, fontsize=13, ncol=1,
           scatterpoints=1)
# -----------------------------------------------------------------------------
...
x_ticks_2 = ["DJF", "MAM", "JJA", "SON"]
for i in prec_prod.keys():
    plt.scatter(x_2, prec_prod_reg[i]["HM"][12:], s=s, color=cols[i], marker=markers[i],
                zorder=3)
plt.xticks(x_2, x_ticks_2, rotation=90, fontsize=13)
plt.yticks(np.arange(0, 11, 1), [""] * 11, fontsize=14)
plt.axis([0.3, 4.7, 0, 11])
# -----------------------------------------------------------------------------
seas = ("DJF", "MAM", "JJA", "SON")
pos_x = (4, 5, 6, 7)
lims = ([0, 16], [0, 16], [0, 16], [0, 16])
for i in range(0, 4):
    lim = lims[i]
    ax = plt.subplot(gs[2, pos_x[i]])
    for j in prec_prod_elev.keys():
        k = seas[i].replace(" ", "_")
        plt.plot(prec_prod_elev[j][k], elev_bin, color=cols[j])
        plt.scatter(prec_prod_elev[j][k], elev_bin, s=s_small, marker=markers[j],
                    color=cols[j], label=j)
    plt.text(x=0.5, y=0.94, s=seas[i], fontsize=14,
             horizontalalignment="center", verticalalignment="center",
             transform=ax.transAxes)
    plt.xticks(np.arange(0, 15, 4), fontsize=14)
    ax.set_xticks(np.arange(0, 15, 4), minor=True)
    if i == 0:
        plt.yticks(np.arange(0, 7000, 500), np.arange(0, 7.0, 0.5), fontsize=14)
        plt.xlabel(" " * 105 + "precipitation [mm $day^{-1}$]", fontsize=14)
        plt.ylabel("Elevation [km a.s.l.]", labelpad=7.5, fontsize=14)
        plt.title("(d)", fontsize=14, fontweight="normal",
                  y=1.01,
                  loc="left")
    else:
        plt.yticks(np.arange(0, 7000, 500), "" * 14)
...
# -----------------------------------------------------------------------------
plt.show()
fig.savefig(path_out + "tmp_pr_elev.png", dpi=300,
           bbox_inches="tight")
plt.close(fig)
