...
for i in ("ctrl", "lgm"):
    ...
    for j in ("DJF", 'MAM', 'JJA', 'SON'):
        ds = xr.open_mfdataset(f'{dir}' + f'EAS11_{i}/szn/TOT_PREC/' + f'2001-2005.TOT_PREC.{j}.nc')
        pr = ds['TOT_PREC'].values[:, :, :]
        data[i][j] = {"pr": pr}

data['diff'] = {}
for j in ("DJF", 'MAM', 'JJA', 'SON'):
    data['diff'][j] = {"pr": data['lgm'][j]['pr'] - data['ctrl'][j]['pr']}
...
for i in range(nrow):
    ...
    for j in range(ncol):
        ...
        cmap = cmaps[j]
        norm = norms[j]
        cs[i, j] = axs[i, j].pcolormesh(rlon, rlat, data[sim][mon]['pr'][0, :, :], cmap=cmap, norm=norm, shading="auto")
...
fig.savefig(plotpath + 'pr_4seasons.png', dpi=500)
plt.close(fig)
