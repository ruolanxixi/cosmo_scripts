...
topo1path = "/scratch/snx3000/rxiang/data/cosmo/EAS11_lgm/szn"
g = 9.80665
...
pr_ctrl = xr.open_dataset(f'{ctrlpath}/{mdvname}/{year}_{mdvname}_{season}.nc')[mdvname].values[0, :, :] * 1000
pr_lgm = xr.open_dataset(f'{topo1path}/{mdvname}/{year}_{mdvname}_{season}.nc')[mdvname].values[0, :, :] * 1000
pr_diff = pr_lgm - pr_ctrl
# -------------------------------------------------------------------------------
# %%
nrow=1
ncol=3
...
cs[0, 0] = axs[0, 0].pcolormesh(rlon, rlat, pr_ctrl, cmap=cmap, norm=norm)
cs[0, 1] = axs[0, 1].pcolormesh(rlon, rlat, pr_lgm, cmap=cmap, norm=norm)
cmap = drywet(25, cmc.vik_r)
cs[0, 2] = axs[0, 2].pcolormesh(rlon, rlat, pr_diff, cmap=cmap, clim=(-10, 10),
                                shading="auto")
...
cb1.set_label('g/kg', fontsize=11)
cb1.ax.tick_params(labelsize=11)
cax2 = fig.add_axes(
    [axs[0, 2].get_position().x0, axs[0, 2].get_position().y0 - .12, axs[0, 2].get_position().width, 0.035])
cb2 = fig.colorbar(cs[0, 2], cax=cax2, orientation='horizontal', extend='both')
cb2.set_label('g/kg', fontsize=11)
...
fig.savefig(plotpath + 'qv2m.png', dpi=300)
plt.close(fig)
