...
tmp_ctrl = xr.open_dataset(f'{ctrlpath}/{mdvname}/{year}_{mdvname}_{season}.nc')[mdvname].values[0, :, :] - 273.15
tmp_lgm = xr.open_dataset(f'{topo1path}/{mdvname}/{year}_{mdvname}_{season}.nc')[mdvname].values[0, :, :] - 273.15
tmp_diff = tmp_lgm - tmp_ctrl
# -------------------------------------------------------------------------------
# %%
nrow=1
ncol=3
...
cs[0, 0] = axs[0, 0].pcolormesh(rlon, rlat, tmp_ctrl, cmap=cmap, norm=norm)
cs[0, 1] = axs[0, 1].pcolormesh(rlon, rlat, tmp_lgm, cmap=cmap, norm=norm)

cmap = custom_div_cmap(27, cmc.vik)
cs[0, 2] = axs[0, 2].pcolormesh(rlon, rlat, tmp_diff, cmap=cmap, clim=(-10, 10),
                                shading="auto")
...
cb1 = fig.colorbar(cs[0, 0], cax=cax1, orientation='horizontal', extend='both')
