...
topo["PD"] = ds["HSURF"].values
ds.close()
# -----------------------------------------------------------------------------
ds = xr.open_dataset("/project/pr133/rxiang/data/extpar/"
                     + "extpar_EAS_ext_12km_merit_LGM_consistent_TCL.nc")
ds = ds.isel(rlon=slice(32, 1090), rlat=slice(28, 638))
topo["LGM"] = ds["HSURF"].values
...
x_ticks_3 = ["Rainy\nseason", "Dry\nseason", "Annual"]
for i in prec_prod.keys():
    plt.scatter(x_3, prec_prod_reg[i]["ET"][12:], s=s, color=cols[i],
                zorder=3)
# plt.fill_between(x=[3.0 - 0.2, 3.0 + 0.2],
#                  y1=prec_prod_reg_unc["PBCOR"]["ET"][0],
#                  y2=prec_prod_reg_unc["PBCOR"]["ET"][1],
#                  color=cols["PBCOR"], facecolor="none", alpha=0.75,
#                  zorder=2)
...
for i in ["HM", "HMC", "HMUN", "HMUS"]:
    ...
    for j in prec_prod.keys():
        plt.scatter(x_3, prec_prod_reg[j][i][12:], s=s, color=cols[j],
                    zorder=3)
    # plt.fill_between(x=[3.0 - 0.2, 3.0 + 0.2],
    ...
    #                  color=cols["PBCOR"], facecolor="none", alpha=0.75,
    #                  zorder=2)
    plt.text(x=1.40, y=20.5, s=i, fontweight="normal")
    plt.axis([0.5, 3.5, 0.0, 19.5])
    plt.xticks(x_3, x_ticks_3, rotation=90, fontsize=9)
    if n == 4:
        plt.yticks(np.arange(0, 24, 2), np.arange(0, 24, 2))
        plt.title("(b) Hengduan Mountains region", fontsize=12,
...
for i in temp_prod.keys():
    ...
    plt.scatter(months_num, temp_prod_reg[i]["ET"][:12], s=s_small,
                color=cols[i], label=i, zorder=3)
plt.fill_between(x=[4.5, 9.5], y1=-4.0, y2=22.0, color="black", alpha=0.1)
plt.xticks(months_num, months_char)
plt.text(x=6.0, y=-3.2, s="Rainy season", fontsize=10)
...
x_ticks_2 = ["Rainy\nseason", "Dry\nseason"]
for i in temp_prod.keys():
    plt.scatter(x_2, temp_prod_reg[i]["ET"][12:], s=s, color=cols[i],
                zorder=3)
plt.xticks(x_2, x_ticks_2, rotation=90, fontsize=9)
plt.yticks(np.arange(-4, 22, 2), [""] * 13)
plt.axis([0.5, 2.5, -4.0, 22.0])
# -----------------------------------------------------------------------------
...
for i in range(0, 2):
    ...
    if i == 0:
        ...
        plt.title("(d) Hengduan Mountains region", fontsize=12, fontweight="normal",
                  y=1.01,
                  loc="left")
    else:
