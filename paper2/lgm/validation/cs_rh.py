...
# # COSMO
ds1 = xr.open_dataset('/scratch/snx3000/rxiang/data/cosmo/EAS11_ctrl/monsoon/QV/01-05.QV.timmean.lonlat.nc')
...
ds2 = xr.open_dataset('/scratch/snx3000/rxiang/data/cosmo/EAS11_lgm/monsoon/QV/01-05.QV.timmean.lonlat.nc')
...
ds3 = xr.open_dataset('/scratch/snx3000/rxiang/data/cosmo/EAS11_ctrl/monsoon/T/01-05.T.timmean.lonlat.nc')
ds3 = ds3.isel(pressure=slice(None, None, -1))
data_var = ds3['T']
meridional_avg = data_var.sel(lat=slice(10, 20)).mean(dim='lat', skipna=True).values
updated_data_var = data_var.copy(deep=True)
updated_data_var.loc[dict(lat=slice(10, 20))] = meridional_avg[:, :, np.newaxis, :]
ds3['T'] = updated_data_var
ds4 = xr.open_dataset('/scratch/snx3000/rxiang/data/cosmo/EAS11_lgm/monsoon/T/01-05.T.timmean.lonlat.nc')
...
cosmo2.name = 'QV2'
cosmo3 = regridder(ds3.T)
cosmo3.name = 'T1'
cosmo4 = regridder(ds4.T)
cosmo4.name = 'T2'

ds_cosmo = xr.merge([cosmo1, cosmo2, cosmo3, cosmo4])
...
cross_cosmo['RH1'] = mpcalc.relative_humidity_from_specific_humidity(cross_cosmo['pressure'] * units.Pa, cross_cosmo['T1'] * units.kelvin, cross_cosmo['QV1'] * units('g/kg'))
cross_cosmo['RH2'] = mpcalc.relative_humidity_from_specific_humidity(cross_cosmo['pressure'] * units.Pa, cross_cosmo['T2'] * units.kelvin, cross_cosmo['QV2'] * units('g/kg'))
# %%
# ECHAM5
# --
ds1 = xr.open_dataset('/project/pr133/rxiang/data/pgw/deltas/native/day/ECHAM5/hur_piControl_timmean.nc')
data_var = ds1['hur']
...
ds1['hur'] = updated_data_var
ds2 = xr.open_dataset('/project/pr133/rxiang/data/pgw/deltas/native/day/ECHAM5/hur_lgm_timmean.nc')
data_var = ds2['hur']
meridional_avg = data_var.sel(lat=slice(20, 10)).mean(dim='lat', skipna=True).values
updated_data_var = data_var.copy(deep=True)
updated_data_var.loc[dict(lat=slice(20, 10))] = meridional_avg[:, :, np.newaxis, :]
ds2['hur'] = updated_data_var
...
target_grid = xe.util.grid_2d(lon0_b=75, lon1_b=165, d_lon=0.11, lat0_b=0., lat1_b=50., d_lat=0.11)
regridder = xe.Regridder(ds1, target_grid, 'bilinear')
echam1 = regridder(ds1.hur)
echam1.name = 'RH1'
echam2 = regridder(ds2.hur)
echam2.name = 'RH2'
...
cross_echam['RH1'] = cross_echam['RH1'] * units.kelvin
cross_echam['RH2'] = cross_echam['RH2'] * units.kelvin

# %%
# PMIP
# --
ds1 = xr.open_dataset('/project/pr133/rxiang/data/pmip/var/hur/hur_Amon_PMIP4_piControl_timmean.nc')
data_var = ds1['hur']
meridional_avg = data_var.sel(lat=slice(10, 20)).mean(dim='lat', skipna=True).values
updated_data_var = data_var.copy(deep=True)
updated_data_var.loc[dict(lat=slice(10, 20))] = meridional_avg[:, :, np.newaxis, :]
...
target_grid = xe.util.grid_2d(lon0_b=75, lon1_b=165, d_lon=0.11, lat0_b=0., lat1_b=50., d_lat=0.11)
regridder = xe.Regridder(ds1, target_grid, 'bilinear')
pmip1 = regridder(ds1.hur)
pmip1.name = 'RH1'
pmip2 = regridder(ds2.hur)
pmip2.name = 'RH2'
...
cross_pmip['RH1'] = cross_pmip['RH1'] * units.kelvin
cross_pmip['RH2'] = cross_pmip['RH2'] * units.kelvin
# %%
...
level1 = np.linspace(0, 100, 20, endpoint=True)
cmap1 = cmc.davos_r
level2 = MaxNLocator(nbins=31).tick_values(-15, 15)
cmap2 = drywet(25, cmc.vik_r)
norm2 = colors.TwoSlopeNorm(vmin=-15., vcenter=0., vmax=15.)

ctf[0, 0] = axs[0, 0].contourf(cross_cosmo['lon'], cross_cosmo['height'], cross_cosmo['RH1'] *100, levels=level1, cmap=cmap1, norm=BoundaryNorm(level1, ncolors=cmap1.N, clip=True), extend='both')
ctf[0, 1] = axs[0, 1].contourf(cross_cosmo['lon'], cross_cosmo['height'], (cross_cosmo['RH2'] - cross_cosmo['RH1']) *100, levels=level2, cmap=cmap2, norm=norm2, extend='both')

ctf[1, 0] = axs[1, 0].contourf(cross_echam['lon'], cross_echam['height'], cross_echam['RH1'] *100, levels=level1, cmap=cmap1, norm=BoundaryNorm(level1, ncolors=cmap1.N, clip=True), extend='both')
ctf[1, 1] = axs[1, 1].contourf(cross_echam['lon'], cross_echam['height'], (cross_echam['RH2'] - cross_echam['RH1']) *100, levels=level2, cmap=cmap2, norm=norm2, extend='both')

ctf[2, 0] = axs[2, 0].contourf(cross_pmip['lon'], cross_pmip['height'], cross_pmip['RH1'], levels=level1, cmap=cmap1, norm=BoundaryNorm(level1, ncolors=cmap1.N, clip=True), extend='both')
ctf[2, 1] = axs[2, 1].contourf(cross_pmip['lon'], cross_pmip['height'], cross_pmip['RH2'] - cross_pmip['RH1'], levels=level2, cmap=cmap2, norm=norm2, extend='both')

ticks = [[0, 20, 40, 60, 80, 100], [-15, -10, -5, 0, 5, 10, 15]]
for i in range(ncol):
    tick = ticks[i]
...
fig.savefig(plotpath + 'cs_rh.png', dpi=500)
plt.close(fig)
