...
for s in range(len(sims)):
    sim = sims[s]
    geopoth[sim], slp[sim] = {}, {}
    geopoth[sim]['label'] = labels[sim]
    for mon in ['01', '07']:
        geopoth[sim][mon] = {}
        name = fname[mon]
        data = xr.open_dataset(f'{mdpath}/{sim}/analysis/geopoth/mon/{name}')
        gp = data['geopoth'].values[0, 3, :, :]/ 10
        geopoth[sim][mon]['geopoth'] = gp
        slp[sim][mon] = {}
        data = xr.open_dataset(f'{mdpath}/{sim}/analysis/slp/mon/{name}')
        sp = ndimage.gaussian_filter(data['slp'].values[0, :, :] / 100, sigma=5, order=0)
        slp[sim][mon]['slp'] = sp

# %%
lat = xr.open_dataset(f'{mdpath}/LGM/analysis/geopoth/mon/{name}')['lat'].values[:]
lon = xr.open_dataset(f'{mdpath}/LGM/analysis/geopoth/mon/{name}')['lon'].values[:]
...
cmap1 = cmc.roma_r
levels1 = np.linspace(540, 600, 13, endpoint=True)
norm1 = BoundaryNorm(levels1, ncolors=cmap1.N, clip=True)

cmap2 = drywet(25, cmc.vik_r)
levels2 = np.linspace(-20, 20, 13, endpoint=True)
...
for mon in ['01', '07']:
    ...
    for i in range(4):
        sim = sims[i]
        label = geopoth[sim]['label']
        axs[0, i] = fig.add_subplot(gs[0, i], projection=ccrs.Robinson(central_longitude=180, globe=None))
        axs[0, i].coastlines(zorder=3)
        axs[0, i].stock_img()
        axs[0, i].gridlines()
        cs[0, i] = axs[0, i].pcolormesh(lon, lat, geopoth[sim][mon]['geopoth'], cmap=cmap1, norm=norm1, shading="auto",
                                        transform=ccrs.PlateCarree())
        ct[0, i] = axs[0, i].contour(lon, lat, slp[sim][mon]['slp'], colors='white', linewidths=1, transform=ccrs.PlateCarree())
        clabel = axs[0, i].clabel(ct[0, i], inline=True, fontsize=13, use_clabeltext=True)
        for l in clabel:
            l.set_rotation(0)
        [txt.set_bbox(dict(facecolor='white', edgecolor='none', pad=0, alpha=0)) for txt in clabel]
        label = geopoth[sim]['label']
    ...
    for i in range(3):
        ...
        axs[1, i + 1].stock_img()
        axs[1, i + 1].gridlines()
        cs[1, i + 1] = axs[1, i + 1].pcolormesh(lon, lat, geopoth[sim][mon]['geopoth'] - geopoth['PI'][mon]['geopoth'], cmap=cmap2,
                                                clim=(-20, 20), shading="auto", transform=ccrs.PlateCarree())
        ct[1, i + 1] = axs[1, i + 1].contour(lon, lat, slp[sim][mon]['slp'] - slp['PI'][mon]['slp'], colors='maroon', levels = [-5, 5, 10, 15, 20, 25], linewidths=1,
                                     transform=ccrs.PlateCarree())
        clabel = axs[1, i + 1].clabel(ct[1, i + 1], [-5, 5, 10, 15, 20, 25], inline=True, fontsize=13, use_clabeltext=True)
        for l in clabel:
            l.set_rotation(0)
    ...
    cbar = fig.colorbar(cs[0, 1], cax=cax, orientation='vertical', extend='both', ticks=np.linspace(540, 600, 4, endpoint=True))
    ...
    axs[0, 0].text(-0.08, 0.5, '500 hPa GPH', ha='center', va='center', rotation='vertical',
    ...
    axs[0, 3].text(1.07, 1.06, '[dam]', ha='center', va='center', rotation='horizontal',
                   transform=axs[0, 3].transAxes, fontsize=12)
    axs[0, 0].text(-0.09, 1.06, f'{m}', ha='left', va='center', rotation='horizontal',
                   transform=axs[0, 0].transAxes, fontsize=13, fontweight='bold')
    axs[0, 0].text(0.5, -0.04, 'Contour: SLP', ha='center', va='top', rotation='horizontal',
                   transform=axs[0, 0].transAxes, fontsize=13, fontweight='bold')

    fig.show()
    plotpath = "/project/pr133/rxiang/figure/echam5/"
    fig.savefig(plotpath + 'geopoth' + f'{mon}.png', dpi=500)
    plt.close(fig)
