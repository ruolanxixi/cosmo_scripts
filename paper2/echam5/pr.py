...
for s in range(len(sims)):
    ...
    for mon in ['01', '07']:
        pr[sim][mon] = {}
        name = fname[mon]
        data = xr.open_dataset(f'{mdpath}/{sim}/analysis/pr/mon/{name}')
        precip = data['precip'].values[0, :, :]*86400
        pr[sim][mon]['precip'] = precip
# %%
lat = xr.open_dataset(f'{mdpath}/LGM/analysis/pr/mon/{name}')['lat'].values[:]
lon = xr.open_dataset(f'{mdpath}/LGM/analysis/pr/mon/{name}')['lon'].values[:]
...
for mon in ['01', '07']:
    ...
    for i in range(4):
        ...
        axs[0, i].stock_img()
        axs[0, i].gridlines()
        cs[0, i] = axs[0, i].pcolormesh(lon, lat, pr[sim][mon]['precip'], cmap=cmap1, norm=norm1, shading="auto", transform=ccrs.PlateCarree())
        # ct[0, i] = axs[0, i].contour(lon, lat, pr[sim]['wssms'], levels=np.linspace(2, 14, 5, endpoint=True),
    ...
    for i in range(3):
        ...
        axs[1, i+1].stock_img()
        axs[1, i+1].gridlines()
        cs[1, i+1] = axs[1, i+1].pcolormesh(lon, lat, pr[sim][mon]['precip'] - pr['PI'][mon]['precip'], cmap=cmap2, clim=(-10, 10), shading="auto", transform=ccrs.PlateCarree())

    cax = fig.add_axes([axs[0, 3].get_position().x1 + 0.01, axs[0, 3].get_position().y0, 0.01, axs[0, 3].get_position().height])
    ...
    cax = fig.add_axes([axs[1, 3].get_position().x1 + 0.01, axs[1, 3].get_position().y0, 0.01, axs[1, 3].get_position().height])
    cbar = fig.colorbar(cs[1, 1], cax=cax, orientation='vertical', extend='both', ticks=np.linspace(-10, 10, 5, endpoint=True))
    cbar.ax.tick_params(labelsize=13)
    cbar.ax.set_xlabel('mm day$^{-1}$', fontsize=13)
    
    axs[0, 0].text(-0.08, 0.5, 'Precipitation', ha='center', va='center', rotation='vertical',
    ...
    axs[0, 3].text(1.07, 1.06, '[mm day$^{-1}$]', ha='center', va='center', rotation='horizontal',
    ...
    fig.savefig(plotpath + 'precip' + f'{mon}.png', dpi=500)
    plt.close(fig)
