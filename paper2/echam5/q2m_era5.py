...
for s in range(len(sims)):
    sim = sims[s]
    spchum2[sim] = {}
    spchum2[sim]['label'] = labels[sim]
    for mon in ['01', '07']:
        spchum2[sim][mon] = {}
        name = fname[mon]
        data = xr.open_dataset(f'{mdpath}/{sim}/analysis/temp2/mon/{name}')
        t2m = data['temp2'].values[0, :, :] * units.kelvin
        dew2 = data['dew2'].values[0, :, :] * units.kelvin
        data = xr.open_dataset(f'{mdpath}/{sim}/analysis/aps/mon/{name}')
        aps = data['aps'].values[0, :, :] * units.pascal
        q2 = mpcalc.specific_humidity_from_dewpoint(aps, dew2) * 1000
        spchum2[sim][mon]['spchum2'] = q2

spchum2['PD']['01']['spchum2'] = mpcalc.specific_humidity_from_dewpoint(xr.open_dataset(f'{erapath}/era5.mo.1970-1995.mon.remap.echam5.nc')['msl'].values[0, :, :] * units.pascal,
                                 xr.open_dataset(f'{erapath}/era5.mo.1970-1995.mon.remap.echam5.nc')['d2m'].values[0, :, :] * units.kelvin) * 1000
spchum2['PD']['07']['spchum2'] = mpcalc.specific_humidity_from_dewpoint(xr.open_dataset(f'{erapath}/era5.mo.1970-1995.mon.remap.echam5.nc')['msl'].values[6, :, :] * units.pascal,
                                 xr.open_dataset(f'{erapath}/era5.mo.1970-1995.mon.remap.echam5.nc')['d2m'].values[6, :, :] * units.kelvin) * 1000
...
for mon in ['01', '07']:
    ...
    for i in range(4):
        sim = sims[i]
        label = spchum2[sim]['label']
        axs[0, i] = fig.add_subplot(gs[0, i], projection=ccrs.Robinson(central_longitude=180, globe=None))
        axs[0, i].coastlines(zorder=3)
        axs[0, i].stock_img()
        axs[0, i].gridlines()
        cs[0, i] = axs[0, i].pcolormesh(lon, lat, spchum2[sim][mon]['spchum2'], cmap=cmap1, norm=norm1, shading="auto",
                                        transform=ccrs.PlateCarree())
        # ct[0, i] = axs[0, i].contour(lon, lat, spchum2[sim]['wssms'], levels=np.linspace(2, 14, 5, endpoint=True),
    ...
    for i in range(3):
        ...
        axs[1, i + 1].stock_img()
        axs[1, i + 1].gridlines()
        cs[1, i + 1] = axs[1, i + 1].pcolormesh(lon, lat, spchum2[sim][mon]['spchum2'] - spchum2['PI'][mon]['spchum2'], cmap=cmap2,
    ...
    axs[0, 0].text(-0.08, 0.5, '2m q$_v$', ha='center', va='center', rotation='vertical',
    ...
    axs[0, 3].text(1.07, 1.06, '[g kg$^{-1}$]', ha='center', va='center', rotation='horizontal',
    ...
    fig.savefig(plotpath + 'spchum2' + f'{mon}.era5.1970-1995.png', dpi=500)
    plt.close(fig)
