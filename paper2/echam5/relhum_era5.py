...
for s in range(len(sims)):
    sim = sims[s]
    relhum[sim] = {}
    relhum[sim]['label'] = labels[sim]
    for mon in ['01', '07']:
        relhum[sim][mon] = {}
        name = fname[mon]
        data = xr.open_dataset(f'{mdpath}/{sim}/analysis/relhum/mon/{name}')
        rh = data['relhum'].values[0, 6, :, :] * 100
        relhum[sim][mon]['relhum'] = rh

relhum['PD']['01']['relhum'] = xr.open_dataset(f'{erapath}/era5.mo.1970-1995.p.mon.remap.echam5.nc')['r'].values[0, 3, :, :]
relhum['PD']['07']['relhum'] = xr.open_dataset(f'{erapath}/era5.mo.1970-1995.p.mon.remap.echam5.nc')['r'].values[6, 3, :, :]
...
for mon in ['01', '07']:
    ...
    for i in range(4):
        sim = sims[i]
        label = relhum[sim]['label']
        axs[0, i] = fig.add_subplot(gs[0, i], projection=ccrs.Robinson(central_longitude=180, globe=None))
        axs[0, i].coastlines(zorder=3)
        axs[0, i].stock_img()
        axs[0, i].gridlines()
        cs[0, i] = axs[0, i].pcolormesh(lon, lat, relhum[sim][mon]['relhum'], cmap=cmap1, norm=norm1, shading="auto",
                                        transform=ccrs.PlateCarree())
        # ct[0, i] = axs[0, i].contour(lon, lat, relhum[sim]['wssms'], levels=np.linspace(2, 14, 5, endpoint=True),
    ...
    for i in range(3):
        ...
        axs[1, i + 1].stock_img()
        axs[1, i + 1].gridlines()
        cs[1, i + 1] = axs[1, i + 1].pcolormesh(lon, lat, relhum[sim][mon]['relhum'] - relhum['PI'][mon]['relhum'], cmap=cmap2,
    ...
    cbar = fig.colorbar(cs[0, 1], cax=cax, orientation='vertical', ticks=np.linspace(0, 100, 6, endpoint=True))
    ...
    axs[0, 0].text(-0.08, 0.5, '1000 hPa RH', ha='center', va='center', rotation='vertical',
    ...
    fig.savefig(plotpath + 'relhum' + f'{mon}.era5.1970-1995.png', dpi=500)
    plt.close(fig)
