...
for s in range(len(sims)):
    ...
    data = Dataset(f'{path}' + '/' + '01-05.U.20000.smr.cpm.nc')
    ...
    data = Dataset(f'{path}' + '/' + '01-05.V.20000.smr.cpm.nc')
...
levels1 = MaxNLocator(nbins=40).tick_values(0, 40)
cmap1 = wind(40, cmc.batlowW_r)
# cmap = cbr_wet(15)
...
for i in range(2):
    ...
    ct[i, 0] = axs[i, 0].contour(rlon, rlat, all_ws_smr_sms[i], levels=np.linspace(5, 40, 8, endpoint=True), colors='maroon', linewidths=1)
    clabel = axs[i, 0].clabel(ct[i, 0], levels=np.linspace(5, 40, 8, endpoint=True), inline=True, fontsize=13, use_clabeltext=True)
    for l in clabel:
        l.set_rotation(0)
    ...
    q[i, 0] = axs[i, 0].quiver(rlon[::40], rlat[::40], all_u_smr[0][::40, ::40],
                  all_v_smr[0][::40, ::40], color='black', scale=200)
...
cs[2, 0] = axs[2, 0].pcolormesh(rlon, rlat, all_ws_smr[1] - all_ws_smr[0], cmap=cmap2, clim=(-5, 5), shading="auto")
ct[2, 0] = axs[2, 0].contour(rlon, rlat, all_ws_smr_sms[1] - all_ws_smr_sms[0],  levels=[-5, -2, 2, 5], colors='maroon',
                       linewidths=1)
...
q[2, 0] = axs[2, 0].quiver(rlon[::40], rlat[::40], all_u_smr[1][::40, ::40]-all_u_smr[0][::40, ::40],
                  all_v_smr[1][::40, ::40]-all_v_smr[0][::40, ::40], color='black', scale=50)
...
cbar = fig.colorbar(cs[1, 0], cax=cax, orientation='horizontal', extend='max', ticks=np.linspace(0, 40, 11, endpoint=True))
