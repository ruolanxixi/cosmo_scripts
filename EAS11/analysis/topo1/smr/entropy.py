...
for s in range(len(sims)):
    ...
    path = f'/project/pr133/rxiang/data/cosmo/EAS11_{sim}/monsoon/PS/smr'
    data = xr.open_dataset(f'{path}' + '/' + '01-05.PS.smr.cpm.nc')
    smr = data['PS'].values[:, :, :]/100
    ps_smr = np.nanmean(smr, axis=0)

    path = f'/project/pr133/rxiang/data/cosmo/EAS11_{sim}/monsoon/QV_2M/smr'
...
levels1 = MaxNLocator(nbins=25).tick_values(280, 330)
cmap1 = cmc.roma_r
# cmap1 = tmpmap(35, cmc.roma_r, 5, 30)
norm1 = BoundaryNorm(levels1, ncolors=cmap1.N, clip=True)

levels2 = MaxNLocator(nbins=25).tick_values(-10, 0)
...
for i in range(2):
    ...
    ct[i, 0] = axs[i, 0].contour(rlon, rlat, all_thetae_smr_sms[i], levels=np.linspace(280, 330, 6, endpoint=True), colors='k', linewidths=.8)
    clabel = axs[i, 0].clabel(ct[i, 0], levels=np.linspace(280, 330, 6, endpoint=True), inline=True, fontsize=11, use_clabeltext=True)
    for l in clabel:
        l.set_rotation(0)
...
cs[2, 0] = axs[2, 0].pcolormesh(rlon, rlat, all_thetae_smr[1] - all_thetae_smr[0], cmap=cmap2, clim=(-10, 0), shading="auto")
ct[2, 0] = axs[2, 0].contour(rlon, rlat, all_thetae_smr_sms[1] - all_thetae_smr_sms[0], [-5, -3, -1], colors='k',
                       linewidths=.8)
clabel = axs[2, 0].clabel(ct[2, 0], [-5, -3, -1], inline=True, use_clabeltext=True, fontsize=11)
for l in clabel:
    l.set_rotation(0)
