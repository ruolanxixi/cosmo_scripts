...
#
sims = ['ctrl', 'topo1']
all_t2m_smr, all_t2m_smr_sms = [], []
all_slp_smr, all_slp_smr_sms = [], []

for s in range(len(sims)):
    sim = sims[s]

    path = f'/project/pr133/rxiang/data/cosmo/EAS11_{sim}/monsoon/T_2M/smr'
    data = Dataset(f'{path}' + '/' + '01-05.T_2M.smr.cpm.nc')
    smr = data.variables['T_2M'][:, :, :] - 273.15
    t2m_smr = np.nanmean(smr, axis=0)
    t2m_smr_sms = ndimage.gaussian_filter(t2m_smr, sigma=10, order=0)
    all_t2m_smr.append(t2m_smr)
    all_t2m_smr_sms.append(t2m_smr_sms)

    path = f'/project/pr133/rxiang/data/cosmo/EAS11_{sim}/monsoon/PMSL/smr'
    data = Dataset(f'{path}' + '/' + '01-05.PMSL.smr.cpm.nc')
    smr = data.variables['PMSL'][:, :, :]/100
    slp_smr = np.nanmean(smr, axis=0)
    slp_smr_sms = ndimage.gaussian_filter(slp_smr, sigma=10, order=0)
    all_slp_smr.append(slp_smr)
    all_slp_smr_sms.append(slp_smr_sms)
...
levels1 = MaxNLocator(nbins=35).tick_values(-5, 30)
cmap1 = cmc.roma_r
cmap1 = tmpmap(35, cmc.roma_r, 5, 30)
norm1 = BoundaryNorm(levels1, ncolors=cmap1.N, clip=True)

levels2 = MaxNLocator(nbins=25).tick_values(-2, 10)
cmap2 = custom_div_cmap(23, cmc.vik)
cmap2 = tmpdiff(27, cmc.vik, 4, 21)
...
for i in range(2):
    sim = sims[i]
    cs[i, 0] = axs[i, 0].pcolormesh(rlon, rlat, all_t2m_smr[i], cmap=cmap1, norm=norm1, shading="auto")
    ct[i, 0] = axs[i, 0].contour(rlon, rlat, all_slp_smr_sms[i], levels=np.linspace(996, 1010, 8, endpoint=True), colors='k', linewidths=.8)
    clabel = axs[i, 0].clabel(ct[i, 0], levels=np.linspace(996, 1010, 8, endpoint=True), inline=True, fontsize=11, use_clabeltext=True)
    for l in clabel:
        l.set_rotation(0)
...
cs[2, 0] = axs[2, 0].pcolormesh(rlon, rlat, all_t2m_smr[1] - all_t2m_smr[0], cmap=cmap2, clim=(-2, 10), shading="auto")
ct[2, 0] = axs[2, 0].contour(rlon, rlat, all_slp_smr_sms[1] - all_slp_smr_sms[0], [-1, -0.5, 0.5, 1, 5], colors='k',
                       linewidths=.8)
clabel = axs[2, 0].clabel(ct[2, 0], [-1, -0.5, 0.5, 1], inline=True, use_clabeltext=True, fontsize=11)
for l in clabel:
    l.set_rotation(0)
