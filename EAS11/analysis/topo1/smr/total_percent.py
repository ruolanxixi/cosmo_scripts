...
cs[2, 0] = axs[2, 0].pcolormesh(rlon, rlat, (all_cpm[1] - all_cpm[0])/all_cpm[0]*100, cmap=cmap2, clim=(-60, 60), shading="auto")
ct[2, 0] = axs[2, 0].contour(rlon, rlat, (all_cpm_sms[1] - all_cpm_sms[0])/all_cpm_sms[0]*100,  levels=[-60, -40, -20, 20, 40, 60], colors='maroon',
                       linewidths=1)
clabel = axs[2, 0].clabel(ct[2, 0], [-60, -40, -20, 20, 40, 60], inline=True, use_clabeltext=True, fontsize=13)
for l in clabel:
    l.set_rotation(0)
...
cs[2, 1] = axs[2, 1].pcolormesh(rlon, rlat, (all_smr[1] - all_smr[0])/all_smr[0]*100, cmap=cmap2, clim=(-60, 60), shading="auto")
ct[2, 1] = axs[2, 1].contour(rlon, rlat, (all_smr_sms[1] - all_smr_sms[0])/all_smr_sms[0]*100, levels=[-60, -40, -20, 20, 40, 60], colors='maroon',
                      linewidths=1)
clabel = axs[2, 1].clabel(ct[2, 1], [-60, -40, -20, 20, 40, 60], inline=True, fontsize=13, use_clabeltext=True)
for l in clabel:
    l.set_rotation(0)
[txt.set_bbox(dict(facecolor='white', edgecolor='none', pad=0, alpha=0.7)) for txt in clabel]
...
cs[2, 2] = axs[2, 2].pcolormesh(rlon, rlat, (all_wtr[1] - all_wtr[0])/all_wtr[0]*100, cmap=cmap2, clim=(-60, 60), shading="auto")
ct[2, 2] = axs[2, 2].contour(rlon, rlat, (all_wtr_sms[1] - all_wtr_sms[0])/all_wtr_sms[0]*100, levels=[-60, -40, -20, 20, 40, 60], colors='maroon',
                      linewidths=1)
clabel = axs[2, 2].clabel(ct[2, 2], [-60, -40, -20, 20, 40, 60], inline=True, fontsize=13, use_clabeltext=True)
for l in clabel:
    l.set_rotation(0)
[txt.set_bbox(dict(facecolor='white', edgecolor='none', pad=0, alpha=0.7)) for txt in clabel]
...
cbar = fig.colorbar(cs[2, 2], cax=cax, orientation='horizontal', extend='both', ticks=np.linspace(-60, 60, 7, endpoint=True))
cbar.ax.tick_params(labelsize=13)
...
fig.savefig(plotpath + 'total_percent.png', dpi=500)
plt.close(fig)
