...
#
sims = ['ctrl', 'topo1']
all_t200_smr, all_t200_smr_sms = [], []
all_gp200_smr, all_gp200_smr_sms = [], []
g = 9.80665
...
for s in range(len(sims)):
    ...
    data = Dataset(f'{path}' + '/' + '01-05.T.20000.smr.cpm.nc')
    smr = data.variables['T'][:, 0, :, :]
    t200_smr = np.nanmean(smr, axis=0)
    t200_smr_sms = ndimage.gaussian_filter(t200_smr, sigma=10, order=0)
    all_t200_smr.append(t200_smr)
    all_t200_smr_sms.append(t200_smr_sms)

    path = f'/project/pr133/rxiang/data/cosmo/EAS11_{sim}/monsoon/FI/smr'
    data = Dataset(f'{path}' + '/' + '01-05.FI.20000.smr.cpm.nc')
    smr = data.variables['FI'][:, 0, :, :]/g
    gp200_smr = np.nanmean(smr, axis=0)
    gp200_smr_sms = ndimage.gaussian_filter(gp200_smr, sigma=10, order=0)
    all_gp200_smr.append(gp200_smr)
    all_gp200_smr_sms.append(gp200_smr_sms)
...
for i in range(2):
    sim = sims[i]
    cs[i, 0] = axs[i, 0].pcolormesh(rlon, rlat, all_t200_smr[i], cmap=cmap1, norm=norm1, shading="auto")
    ct[i, 0] = axs[i, 0].contour(rlon, rlat, all_gp200_smr_sms[i], levels=np.linspace(11700, 12600, 10, endpoint=True), colors='k', linewidths=.8)
    clabel = axs[i, 0].clabel(ct[i, 0], levels=np.linspace(11700, 12600, 10, endpoint=True), inline=True, fontsize=11, use_clabeltext=True)
    for l in clabel:
        l.set_rotation(0)
...
cs[2, 0] = axs[2, 0].pcolormesh(rlon, rlat, all_t200_smr[1] - all_t200_smr[0], cmap=cmap2, clim=(-2, 2), shading="auto")
ct[2, 0] = axs[2, 0].contour(rlon, rlat, all_gp200_smr_sms[1] - all_gp200_smr_sms[0], [-30, -20, -10, 5, 10, 15], colors='k',
                       linewidths=.8)
clabel = axs[2, 0].clabel(ct[2, 0], [-30, -20, -10, 5, 10, 15], inline=True, use_clabeltext=True, fontsize=11)
for l in clabel:
    l.set_rotation(0)
...
cbar = fig.colorbar(cs[1, 0], cax=cax, orientation='horizontal', extend='both', ticks=np.linspace(216, 226, 6, endpoint=True))
...
axs[0, 0].set_title("200 hPa T & Geopotential height", fontweight='bold', pad=7, fontsize=13, loc='left')
...
fig.savefig(plotpath + 't200.png', dpi=500)
plt.close(fig)
