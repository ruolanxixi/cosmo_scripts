...
for s in range(len(sims)):
    sim = sims[s]
    path = f'/project/pr133/rxiang/data/cosmo/EAS11_{sim}/monsoon/ASHFL_S/wtr'
    data = Dataset(f'{path}' + '/' + '01-05.ASHFL_S.wtr.cpm.nc')
    wtr = - data.variables['ASHFL_S'][...]
    wtr = np.nanmean(wtr, axis=0)
    wtr_sms = ndimage.gaussian_filter(wtr, sigma=10, order=0)
    all_sh.append(wtr)
    all_sh_sms.append(wtr_sms)
...
levels1 = np.linspace(-20, 120, 15, endpoint=True)
cmap1 = cmc.roma_r
# cmap = cbr_wet(15)
...
for i in range(2):
    ...
    ct[i, 0] = axs[i, 0].contour(rlon, rlat, all_sh_sms[i], levels=np.linspace(-20, 120, 8, endpoint=True), colors='k', linewidths=.8)
    clabel = axs[i, 0].clabel(ct[i, 0], levels=np.linspace(-20, 120, 8, endpoint=True), inline=True, fontsize=11, use_clabeltext=True)
    for l in clabel:
        l.set_rotation(0)
...
ct[2, 0] = axs[2, 0].contour(rlon, rlat, all_sh_sms[1] - all_sh_sms[0], [-10, -5, 5, 10], colors='k',
                       linewidths=.8)
clabel = axs[2, 0].clabel(ct[2, 0], [-10, -5, 5, 10], inline=True, use_clabeltext=True, fontsize=11)
for l in clabel:
    l.set_rotation(0)
