...
for s in range(len(sims)):
    ...
    data = Dataset(f'{path}' + '/' + '01-05.U.20000.wtr.cpm.nc')
    ...
    data = Dataset(f'{path}' + '/' + '01-05.V.20000.wtr.cpm.nc')
...
levels1 = MaxNLocator(nbins=20).tick_values(0, 80)
cmap1 = wind(20, cmc.batlowW_r)
# cmap = cbr_wet(15)
...
for i in range(2):
    ...
    ct[i, 0] = axs[i, 0].contour(rlon, rlat, all_ws_wtr_sms[i], levels=np.linspace(10, 70, 7, endpoint=True), colors='maroon', linewidths=1)
    clabel = axs[i, 0].clabel(ct[i, 0], levels=np.linspace(10, 70, 7, endpoint=True), inline=True, fontsize=13, use_clabeltext=True)
    for l in clabel:
        l.set_rotation(0)
    ...
    q[i, 0] = axs[i, 0].quiver(rlon[::40], rlat[::40], all_u_wtr[0][::40, ::40],
                  all_v_wtr[0][::40, ::40], color='black', scale=300)

qk[0, 0] = axs[0, 0].quiverkey(q[0, 0], 0.88, 1.06, 10, r'$10\ \frac{m}{s}$', labelpos='E', transform=axs[0, 0].transAxes,
                      fontproperties={'size': 12})
# plot difference
axs[2, 0] = plotcosmo(axs[2, 0])
cs[2, 0] = axs[2, 0].pcolormesh(rlon, rlat, all_ws_wtr[1] - all_ws_wtr[0], cmap=cmap2, clim=(-5, 5), shading="auto")
ct[2, 0] = axs[2, 0].contour(rlon, rlat, all_ws_wtr_sms[1] - all_ws_wtr_sms[0],  levels=[-5, -2, 2, 5], colors='maroon',
                       linewidths=1)
clabel = axs[2, 0].clabel(ct[2, 0], [-5, -2, 2, 5], inline=True, use_clabeltext=True, fontsize=13)
for l in clabel:
    l.set_rotation(0)
...
q[2, 0] = axs[2, 0].quiver(rlon[::40], rlat[::40], all_u_wtr[1][::40, ::40]-all_u_wtr[0][::40, ::40],
                  all_v_wtr[1][::40, ::40]-all_v_wtr[0][::40, ::40], color='black', scale=50)
qk[2, 0] = axs[2, 0].quiverkey(q[2, 0], 0.9, 1.06, 2, r'$2\ \frac{m}{s}$', labelpos='E', transform=axs[2, 0].transAxes,
                      fontproperties={'size': 12})

cax = fig.add_axes([axs[1, 0].get_position().x0, axs[1, 0].get_position().y0 - .05, axs[1, 0].get_position().width, 0.02])
cbar = fig.colorbar(cs[1, 0], cax=cax, orientation='horizontal', extend='max', ticks=np.linspace(0, 80, 9, endpoint=True))
...
cbar.ax.tick_params(labelsize=13)
cbar.ax.set_xlabel('mm s$^{-1}$', fontsize=13)

axs[0, 0].set_title("200 hPa Wind", fontweight='bold', pad=7, fontsize=13, loc='left')
...
fig.savefig(plotpath + 'wind200.png', dpi=500)
plt.close(fig)
