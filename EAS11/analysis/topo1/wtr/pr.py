...
for i in range(2):
    ...
    ct[i, 0] = axs[i, 0].contour(rlon, rlat, all_pr_sms[i], levels=np.linspace(2, 14, 5, endpoint=True), colors='k', linewidths=.8)
...
ct[2, 0] = axs[2, 0].contour(rlon, rlat, all_pr_sms[1] - all_pr_sms[0], levels=[-2, -1, 1, 2], colors='k',
                       linewidths=.8)
...
axs[0, 0].set_title("Total rainfall Nov to Mar", fontweight='bold', pad=18, fontsize=14)
