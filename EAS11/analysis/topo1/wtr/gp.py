...
#
sims = ['ctrl', 'topo1']
all_gp_wtr, all_gp_wtr_sms = [], []
g = 9.80665

for s in range(len(sims)):
    sim = sims[s]

    path = f'/project/pr133/rxiang/data/cosmo/EAS11_{sim}/monsoon/FI/wtr'
    data = Dataset(f'{path}' + '/' + '01-05.FI.50000.wtr.cpm.nc')
    wtr = data.variables['FI'][:, 0, :, :]/g
    gp_wtr = np.nanmean(wtr, axis=0)
    all_gp_wtr.append(gp_wtr)
...
levels1 = MaxNLocator(nbins=16).tick_values(5100, 5900)
cmap1 = cmc.roma_r
# cmap = cbr_wet(15)
...
for i in range(2):
    sim = sims[i]
    cs[i, 0] = axs[i, 0].pcolormesh(rlon, rlat, all_gp_wtr[i], cmap=cmap1, norm=norm1, shading="auto")
    ct[i, 0] = axs[i, 0].contour(rlon, rlat, all_gp_wtr[i], levels=np.linspace(5150, 5850, 15, endpoint=True), colors='k', linewidths=.8)
    clabel = axs[i, 0].clabel(ct[i, 0], levels=np.linspace(5150, 5850, 15, endpoint=True), inline=True, fontsize=11, use_clabeltext=True)
    for l in clabel:
        l.set_rotation(0)
...
cs[2, 0] = axs[2, 0].pcolormesh(rlon, rlat, all_gp_wtr[1] - all_gp_wtr[0], cmap=cmap2, norm=divnorm, shading="auto")
ct[2, 0] = axs[2, 0].contour(rlon, rlat, all_gp_wtr[1] - all_gp_wtr[0], levels=np.linspace(-24, 12, 13, endpoint=True), colors='k',
                       linewidths=.8)
...
cbar = fig.colorbar(cs[1, 0], cax=cax, orientation='horizontal', extend='both', ticks=np.linspace(5100, 5900, 5, endpoint=True))
