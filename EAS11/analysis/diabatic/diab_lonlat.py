...
class Plot_Cross(UserDict):
    ...
    def add_terrain(self, xyline, level=57):
        # FIXME
        f = Dataset('/project/pr133/rxiang/data/cross/terrain.ctrl.zonmean.nc')
        hsurf = f.variables['HSURF'][0, ...]

        hsurf = np.repeat(hsurf[np.newaxis, ...], level, axis=0)

        terrain = wrf.interp2dxy(hsurf, xyline)[1]
        self.ax.plot(np.arange(terrain.shape[0]), terrain / 1000, color='black', linewidth=0.01)
        self.ax.fill_between(np.arange(terrain.shape[0]), terrain / 1000, color='black')
...
if __name__ == '__main__':
    data_d = Dataset('/project/pr133/rxiang/data/cosmo/EAS11_ctrl/szn/DIAB_SUM/2001-2005.DIAB_SUM.JJA.zonmean.nc')
