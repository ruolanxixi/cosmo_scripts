...
for i, j in zip(lonmin, lonmax):
    ...
    # plot
    ar = 3.4  # initial aspect ratio for first trial
    hi = 5.5  # height in inches
    ...
    left, bottom, right, top = 0.05, 0.2, 0.99, 0.93
    gs = gridspec.GridSpec(nrows=1, ncols=4, width_ratios=[.7, 4, 4, 4], left=left, bottom=bottom, right=right, top=top, wspace=0.2, hspace=0.1)

    y_tick_labels = [u'0\N{DEGREE SIGN}N', u'5\N{DEGREE SIGN}N', u'10\N{DEGREE SIGN}N', u'15\N{DEGREE SIGN}N', u'20\N{DEGREE SIGN}E', u'25\N{DEGREE SIGN}N',
    ...
    ax1.set_extent([i, j, 0, 50], ccrs.PlateCarree(central_longitude=180))
    ax1.set_xticks([i, j])
    ax1.set_xticklabels([f'{i}°N', f'{j}°N'])
    ...
    cf4 = ax4.pcolormesh(time_, lat_, np.transpose(cpm_ctrl[:, :, 0]) - np.transpose(cpm_topo1[:, :, 0]), cmap=cmc.vik_r)
    ct2 = ax2.contour(time_, lat_, mpcalc.smooth_n_point(np.transpose(cpm_ctrl[:, :, 0]), 5, 1), colors='k', linewidths=1)
    ct3 = ax3.contour(time_, lat_, mpcalc.smooth_n_point(np.transpose(cpm_topo1[:, :, 0]), 5, 1), colors='k',
                      linewidths=1)
    ct4 = ax4.contour(time_, lat_, mpcalc.smooth_n_point(np.transpose(cpm_ctrl[:, :, 0]) - np.transpose(cpm_topo1[:, :, 0]), 9, 1), colors='k',
                      linewidths=1)

    cax = fig.add_axes([ax2.get_position().x0, ax2.get_position().y0 - 0.1, ax2.get_position().width*2+0.04, 0.02])
    cbar1 = fig.colorbar(cf2, cax=cax, orientation='horizontal', extend='max')
    cbar1.ax.tick_params(labelsize=13)
    cbar1.ax.set_xlabel('mm/day', fontsize=13, labelpad=-0.01)

    cax = fig.add_axes(
        [ax4.get_position().x0, ax4.get_position().y0 - 0.1, ax4.get_position().width, 0.02])
    cbar2 = fig.colorbar(cf4, cax=cax, orientation='horizontal', extend='both')
    cbar2.ax.tick_params(labelsize=13)
    cbar2.ax.set_xlabel('mm/day', fontsize=13, labelpad=-0.01)

    # Make some ticks and tick labels
    for i in ax2, ax3, ax4:
        i.set_xticks(time[3::6])
        i.set_xticklabels(['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'])
        i.set_ylim(0, 50)
        i.set_yticks([0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50])
        i.set_yticklabels(y_tick_labels)
        i.grid(linestyle='dotted', linewidth=2)
