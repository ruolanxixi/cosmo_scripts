...
ct = np.empty(shape=(2, 2), dtype='object')
...
topo_ctrl[topo_ctrl <= 5500] = 'nan'
topo_topo1[topo_topo1 <= 5500] = 'nan'
levels = MaxNLocator(nbins=40).tick_values(-20, 3)
cmap = cmc.roma_r
...
cax1 = fig.add_axes(
    [axs[0, 0].get_position().x0, axs[0, 0].get_position().y0 - 0.06, axs[0, 0].get_position().width * 2.12, 0.02])
...
cs[1, 0] = axs[1, 0].pcolormesh(rlon, rlat, da.sel(sim='diff').values[:, :], cmap=cmap, clim=(-1.2, 1.2),
                                shading="auto")
...
axs[1, 0].set_title('Reduced elevation - Control', fontweight='bold', pad=12, fontsize=13)


cb1.set_label('m', fontsize=12)
cax2 = fig.add_axes(
    [axs[1, 0].get_position().x0, axs[1, 0].get_position().y0 - 0.06, axs[1, 0].get_position().width, 0.02])
