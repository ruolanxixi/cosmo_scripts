...
cmap1 = windmap(15, cmc.batlowW_r)
norm1 = BoundaryNorm(levels1, ncolors=cmap1.N, clip=True)

levels2 = MaxNLocator(nbins=15).tick_values(-10, 10)
...
for mon in ['01', '07']:
    ...
    for i in range(4):
        ...
        q[0, i] = axs[0, i].quiver(lon[::10], lat[::10], wind[sim][mon]['u'][::10, ::10],
                                   wind[sim][mon]['v'][::10, ::10], color='black', scale=200, transform=ccrs.PlateCarree())
        label = wind[sim]['label']
    ...
    for i in range(3):
        ...
        axs[1, i+1].stock_img()
        axs[1, i+1].gridlines()
        cs[1, i+1] = axs[1, i+1].pcolormesh(lon, lat, wind[sim][mon]['ws'] - wind['PI'][mon]['ws'], cmap=cmap2, norm=norm2, shading="auto",
                                        transform=ccrs.PlateCarree())
        q[1, i+1] = axs[1, i+1].quiver(lon[::10], lat[::10], wind[sim][mon]['u'][::10, ::10] - wind['PI'][mon]['u'][::10, ::10],
                                   wind[sim][mon]['v'][::10, ::10] - wind['PI'][mon]['v'][::10, ::10], color='black',
                                       scale=150, transform=ccrs.PlateCarree())
    ...
    axs[0, 0].text(-0.08, 0.5, '850 hPa Wind', ha='center', va='center', rotation='vertical',
    ...
    fig.savefig(plotpath + 'wind' + f'{mon}.png', dpi=500)
    plt.close(fig)
