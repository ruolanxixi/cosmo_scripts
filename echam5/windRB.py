...
q[0, 0] = axs[0, 0].quiver(lon1[::4], lat1[::4], wind850['PD']['u'][::4, ::4],
                           wind850['PD']['v'][::4, ::4], color='black', scale=150, transform=ccrs.PlateCarree())
...
for l in clabel:
    l.set_rotation(0)
[txt.set_bbox(dict(facecolor='white', edgecolor='none', pad=0, alpha=0.5)) for txt in clabel]
q[1, 0] = axs[1, 0].quiver(lon1[::4], lat1[::4], wind200['PD']['u'][::4, ::4],
                           wind200['PD']['v'][::4, ::4], color='black', scale=500, transform=ccrs.PlateCarree())
...
for j in range(3):
    ...
    for l in clabel:
        l.set_rotation(0)
    [txt.set_bbox(dict(facecolor='white', edgecolor='none', pad=0, alpha=0.5)) for txt in clabel]
    q[0, j+1] = axs[0, j+1].quiver(lon2[::10], lat2[::10], wind850[sim]['u'][::10, ::10],
                               wind850[sim]['v'][::10, ::10], color='black', scale=150, transform=ccrs.PlateCarree())
    ...
    for l in clabel:
        l.set_rotation(0)
    [txt.set_bbox(dict(facecolor='white', edgecolor='none', pad=0, alpha=0.5)) for txt in clabel]
    q[1, j+1] = axs[1, j+1].quiver(lon2[::10], lat2[::10], wind200[sim]['u'][::10, ::10],
                               wind200[sim]['v'][::10, ::10], color='black', scale=500, transform=ccrs.PlateCarree())

qk[0, 3] = axs[0, 3].quiverkey(q[0, 3], 0.88, 1.08, 10, r'$10\ \frac{m}{s}$', labelpos='E', transform=axs[0, 3].transAxes,
                      fontproperties={'size': 12})
qk[1, 3] = axs[1, 3].quiverkey(q[1, 3], 0.88, 1.08, 20, r'$20\ \frac{m}{s}$', labelpos='E', transform=axs[0, 3].transAxes,
                      fontproperties={'size': 12})
...
axs[0, 0].text(-0.1, 0.5, '850 hPa wind', ha='center', va='center', rotation='vertical',
               transform=axs[0, 0].transAxes, fontsize=13, fontweight='bold')
axs[1, 0].text(-0.1, 0.5, '200 hPa wind', ha='center', va='center', rotation='vertical',
