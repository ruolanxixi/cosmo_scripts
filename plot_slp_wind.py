...
file1 = '01_PMSL_mergetime_DJF.nc'
file2 = '01_U_mergetime_85000_DJF.nc'
file3 = '01_V_mergetime_85000_DJF.nc'

ds = xr.open_dataset(path1 + file1)
prec_ctrl = ds["PMSL"].values[0, :, :]/100
lat = ds["lat"].values
lon = ds["lon"].values
ds.close()

ds = xr.open_dataset(path2 + file1)
prec_topo1 = ds["PMSL"].values[0, :, :]/100
ds.close()
...
cs0 = axs0.contourf(lon, lat, prec_ctrl, transform=ccrs.PlateCarree(), levels=np.linspace(1020, 1036, 17), cmap='YlGnBu', vmin=1020, vmax=1036, extend='both')
# axs0.contour(lon, lat, prec_ctrl, cs0.levels, colors='k', linewidths=.3)
cs1 = axs1.contourf(lon, lat, prec_topo1, transform=ccrs.PlateCarree(), levels=np.linspace(1020, 1036, 17), cmap='YlGnBu', vmin=1020, vmax=1036, extend='both')
# axs1.contour(lon, lat, prec_topo1, cs1.levels, colors='k', linewidths=.3)
cs2 = axs2.contourf(lon, lat, prec_diff, transform=ccrs.PlateCarree(), levels=np.linspace(-1.5, 1.5, 16), cmap='RdYlBu', vmin=-1, vmax=1)
