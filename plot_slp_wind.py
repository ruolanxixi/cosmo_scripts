...
file1 = '01_PMSL_DJF_cut.nc'
file2 = '01_U_85000_DJF_cut.nc'
file3 = '01_V_85000_DJF_cut.nc'

ds = xr.open_dataset(path1 + file1)
slp_ctrl = ds["PMSL"].values[0, :, :]/100
lat = ds["lat"].values
lon = ds["lon"].values
ds.close()

ds = xr.open_dataset(path2 + file1)
slp_topo1 = ds["PMSL"].values[0, :, :]/100
ds.close()

slp_diff = slp_ctrl - slp_topo1
...
cs0 = axs0.contourf(lon, lat, slp_ctrl, transform=ccrs.PlateCarree(), levels=np.linspace(1020, 1036, 17), cmap='YlOrRd', vmin=1020, vmax=1036, extend='both')
# axs0.contour(lon, lat, slp_ctrl, cs0.levels, colors='k', linewidths=.3)
cs1 = axs1.contourf(lon, lat, slp_topo1, transform=ccrs.PlateCarree(), levels=np.linspace(1020, 1036, 17), cmap='YlOrRd', vmin=1020, vmax=1036, extend='both')
# axs1.contour(lon, lat, slp_topo1, cs1.levels, colors='k', linewidths=.3)
divnorm=colors.TwoSlopeNorm(vmin=-6., vcenter=0., vmax=3)
cs2 = axs2.contourf(lon, lat, slp_diff, transform=ccrs.PlateCarree(), levels=np.linspace(-6, 3, 19), cmap='RdYlBu_r', norm=divnorm, extend='both')
# axs2.contour(lon, lat, slp_diff, cs2.levels, colors='k', linewidths=.3)
...
fig.savefig('figure_slp_wind.png', dpi=300)
