...
dates = ['01 May - 10 May', '11 May - 20 May', '21 May - 31 May',
...
for i in range(len(sims)):
    ...
    for j in np.arange(1, 16):
        ...
        date = dates[j-1]
        # read data
        data = readdata("TOT_PREC", f'{jj}', f'{sim}')
        data_u = readdata("TWATFLXU", f'{jj}', f'{sim}')
        data_v = readdata("TWATFLXV", f'{jj}', f'{sim}')
        # plot
        ...
        q = ax.quiver(rlon[::30], rlat[::30], data_u[::30, ::30], data_v[::30, ::30], color='black', scale=5000)
