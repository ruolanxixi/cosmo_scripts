...
from mycolor import custom_div_cmap, custom_seq_cmap, custom_white_cmap
...
file1 = 'extpar_BECCY_4.4km_merit_unmod_topo.nc'
file2 = 'extpar_BECCY_4.4km_merit_reduced_topo.nc'
file3 = 'extpar_BECCY_4.4km_merit_env_topo.nc'
...
ds = xr.open_dataset('/scratch/snx3000/rxiang/EXTPAR/output/EAS_4.4km_710x710/extpar_BECCY_4.4km_merit.nc')
...
elev_diff1 = np.ma.masked_where(elev_diff1 < .5, elev_diff1)
elev_diff2 = elev_topo2 - elev_ctrl
elev_diff2 = np.ma.masked_where(elev_diff2 < .5, elev_diff2)

levels1 = np.arange(0., 6500.0, 500.0)
ticks1 = np.arange(0., 6500.0, 500.0)
cmap1 = truncate_colormap(cm.bukavu, 0.55, 1.0)
norm1 = BoundaryNorm(levels1, ncolors=cmap1.N, extend="max")

levels2 = np.arange(0., 2750.0, 250.0)
ticks2 = np.arange(0., 2750.0, 500.0)
cmap2 = cm.lajolla
...
fig = plt.figure(figsize=(11, 13), constrained_layout=True)
axs = np.empty(shape=(2, 2), dtype='object')
cs = np.empty(shape=(2, 2), dtype='object')
...
axs[0, 0] = plt.subplot(gs1[0], projection=ccrs.PlateCarree())
axs[0, 1] = plt.subplot(gs1[1], projection=ccrs.PlateCarree())
axs[1, 0] = plt.subplot(gs2[0], projection=ccrs.PlateCarree())
axs[1, 1] = plt.subplot(gs2[1], projection=ccrs.PlateCarree())
...
cs[1, 1] = axs[1, 1].pcolormesh(lon, lat, elev_diff1, transform=ccrs.PlateCarree(), cmap=cmap2, norm=norm2)  #comment
...
cax1 = fig.add_axes([axs[0, 0].get_position().x0, axs[0, 0].get_position().y0 - 0.06, axs[0, 0].get_position().width * 2.12, 0.02])
cb1 = fig.colorbar(cs[0, 0], cax=cax1, orientation='horizontal', extend='max', ticks=ticks1)
cb1.set_label('m', fontsize=12)
cax2 = fig.add_axes([axs[1, 0].get_position().x0, axs[1, 0].get_position().y0 - 0.06, axs[1, 0].get_position().width * 2.12, 0.02])
cb2 = fig.colorbar(cs[1, 0], cax=cax2, orientation='horizontal', extend='max', ticks=ticks2)
cb2.set_label('m', fontsize=12)

plt.show()
# plotpath = "/project/pr133/rxiang/figure/topo/"
# fig.savefig(plotpath + 'topo_scenarios_4.4km.png', dpi=300)
# plt.close(fig)

# plot 4.4km
# ar = 1.1  # initial aspect ratio for first trial
# wi = 4.5  # height in inches
# hi = wi * ar  # width in inches
# fig = plt.figure(figsize=(wi, hi))
# left, bottom, right, top = 0.1, 0.2, 0.995, 0.95
# gs = gridspec.GridSpec(1, 1, left=left, bottom=bottom, right=right, top=top, wspace=0.2, hspace=0.1)
...
# cb.set_ticks(np.arange(0, 6001, 1000))
# cb.set_label('m', fontsize=13)
# # adjust figure
# fig.show()
# # save figure
# plotpath = "/project/pr133/rxiang/figure/topo/"
# fig.savefig(plotpath + 'topo_ctrl_4.4km.png', dpi=300, transparent=False)
# plt.close(fig)
