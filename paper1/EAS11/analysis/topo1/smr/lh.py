...
for i in range(2):
    ...
    ct[i, 0] = axs[i, 0].contour(rlon, rlat, all_sh_sms[i], levels=np.linspace(0, 200, 11, endpoint=True), colors='k', linewidths=.8)
    clabel = axs[i, 0].clabel(ct[i, 0], levels=np.linspace(0, 200, 11, endpoint=True), inline=True, fontsize=11, use_clabeltext=True)
    for l in clabel:
        l.set_rotation(0)
...
cs[2, 0] = axs[2, 0].pcolormesh(rlon, rlat, all_sh[1] - all_sh[0], cmap=cmap2, clim=(-40, 40), shading="auto")
ct[2, 0] = axs[2, 0].contour(rlon, rlat, all_sh_sms[1] - all_sh_sms[0], [-20, -10, 10, 20], colors='k',
                       linewidths=.8)
