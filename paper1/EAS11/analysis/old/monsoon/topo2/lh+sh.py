...
for s in range(len(sims)):
    ...
    cpm = np.nanmean(cpm[18:24], axis=0)
...
ds.close()
ds = xr.open_dataset('/project/pr133/rxiang/data/extpar/extpar_EAS_ext_12km_merit_env_topo.nc')
...
fig.suptitle('Latent Heat Flux (April)', fontsize=16, fontweight='bold')
...
fig.savefig(plotpath + 'lh_apr.png', dpi=500)
plt.close(fig)
...
fig.suptitle('Sensible Heat Flux (April)', fontsize=16, fontweight='bold')
...
fig.show()
plotpath = "/project/pr133/rxiang/figure/EAS11/analysis/monsoon/topo2/"
fig.savefig(plotpath + 'sh_apr.png', dpi=500)
plt.close(fig)
