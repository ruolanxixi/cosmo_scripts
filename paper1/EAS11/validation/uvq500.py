...
wind = {}
labels = ['CTRL11', 'ERA5', 'CTRL11 - ERA5']
...
for s in range(len(seasons)):
    ...
    v = data['V'].values[0, 0, :, :]
    data = xr.open_dataset(f'{mdpath}/QV/2001-2005.QV.50000.{season}.nc')
...
levels1 = MaxNLocator(nbins=12).tick_values(0, 0.006)
cmap1 = plt.cm.get_cmap('YlGnBu')
norm1 = BoundaryNorm(levels1, ncolors=cmap1.N, clip=True)

levels2 = MaxNLocator(nbins=21).tick_values(-0.002, 0.002)
cmap2 = custom_div_cmap(21, cmc.broc_r)
norm2 = matplotlib.colors.Normalize(vmin=-0.002, vmax=0.002)
...
left, bottom, right, top = 0.07, 0.115, 0.985, 0.97
...
cbar = fig.colorbar(cs[3, 1], cax=cax, orientation='horizontal', extend='max',
                    ticks=np.linspace(0, 0.006, 4, endpoint=True))
cbar.ax.tick_params(labelsize=14)
cbar.ax.set_xlabel('kg kg$^{-1}$', fontsize=14, labelpad=-0.02)

cax = fig.add_axes(
    [axs[3, 2].get_position().x0, axs[3, 2].get_position().y0 - 0.045, axs[3, 2].get_position().width, 0.015])
cbar = fig.colorbar(cs[3, 2], cax=cax, orientation='horizontal', extend='both',
                    ticks=np.linspace(-0.002, 0.002, 5, endpoint=True))
cbar.ax.tick_params(labelsize=14)
cbar.ax.set_xlabel('kg kg$^{-1}$', fontsize=14, labelpad=-0.02)
...
fig.savefig(plotpath + 'uvq500.png', dpi=500, transparent=True)
plt.close(fig)
