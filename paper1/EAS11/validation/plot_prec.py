...
for seas in range(len(seasons)):
    season = seasons[seas]
    filename = f'era5.mo.2001.{season}.remap.11.nc'
...
for seas in range(len(seasons)):
    season = seasons[seas]
    filename = f'cru.2001.05.{season}.remap.11.nc'
    data = xr.open_dataset(f'{crupath}{filename}')['pre'].values[0, :, :]
    otdata.append(data)

for seas in range(len(seasons)):
    season = seasons[seas]
    filename = f'IMERG.2001.{season}.remap.11.nc4'
...
fig.savefig(plotpath + 'pr04.png', dpi=300)
plt.close(fig)
