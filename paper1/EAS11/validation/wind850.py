...
for i in range(len(seasons)):
    ...
    for j in range(3):
        ...
        cs[i, j] = axs[i, j].pcolormesh(rlon, rlat, wind[sim][season]['ws'], cmap=cmap, norm=norm, shading="auto")
        q[i, j] = axs[i, j].quiver(rlon[::40], rlat[::40], wind[sim][season]['u'][::40, ::40],
                                   wind[sim][season]['v'][::40, ::40], color='black', scale=scale)

qk[0, 1] = axs[0, 1].quiverkey(q[0, 1], 0.9, 1.06, 10, r'$10$', labelpos='E', transform=axs[0, 1].transAxes,
                               fontproperties={'size': 14})
qk[0, 2] = axs[0, 2].quiverkey(q[0, 2], 0.94, 1.06, 5, r'$5$', labelpos='E', transform=axs[0, 2].transAxes,
                               fontproperties={'size': 14})
...
cbar.ax.set_xlabel('m s$^{-1}$', fontsize=14, labelpad=-0.02)
...
fig.savefig(plotpath + 'wind850.png', dpi=500, transparent=True)
plt.close(fig)
