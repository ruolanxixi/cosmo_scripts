...
topo1path = "/project/pr133/rxiang/data/cosmo/EAS11_topo2/szn/"
sims = ['ctrl', 'ctrl']
g = 9.80665
...
ct = ax.contour(rlon, rlat, da_f.sel(sim='diff').values[:, :] / g, levels=np.linspace(-15, 15, 11, endpoint=True),
                             colors='maroon',
                             linewidths=.8)
ax.clabel(ct, ct.levels[::1], inline=True, fontsize=8)
ax.text(0.21, 1.05, 'Envelope topography - Control', ha='center', va='center', transform=ax.transAxes, fontsize=10)
...
fig.show()
# save figure
plotpath = "/project/pr133/rxiang/figure/topo2/"
