...
for s in range(len(seasons)):
    ...
    # COSMO 4.4 km
    precip['CPM'][season] = {}
    data = xr.open_dataset(f'{mdpath04}/2001-2005.TOT_PREC.hr.{season}.intens.nc')
    ...
    # COSMO 12 km
    precip['LSM'][season] = {}
    data = xr.open_dataset(f'{mdpath}/2001-2005.TOT_PREC.hr.{season}.intens.nc')
    pr = data['TOT_PREC'].values[0, :, :]
    ...
    # ERA5
    precip['ERA5'][season] = {}
    data = xr.open_dataset(f'{erapath}/freq/2001-2005.hr.{season}.intens.nc')
    ...
    # ERA5
    precip['IMERG'][season] = {}
    data = xr.open_dataset(f'{imergpath}/freq/2001-2005.hr.{season}.intens.nc')
    ...
    # --
    data = xr.open_dataset(f'{imergpath}/remap/2001-2005.hr.{season}.intens.remap.04.nc')
    pr = data['precip'].values[0, :, :]
    precip['CPM'][season]['R']['IMERG'] = \
        ma.corrcoef(ma.masked_invalid(precip['CPM'][season]['precip'][87:-10, 160:-10].flatten()), ma.masked_invalid(pr[87:-10, 160:-10].flatten()))[0, 1]
    precip['CPM'][season]['BIAS']['IMERG'] = np.nanmean(
        precip['CPM'][season]['precip'][87:-10, 160:-10] - pr[87:-10, 160:-10])
    data = xr.open_dataset(f'{imergpath}/remap/2001-2005.hr.{season}.intens.remap.nc')
    pr = data['precip'].values[0, :, :]
...
levels = MaxNLocator(nbins=20).tick_values(0, 5)
cmap = cmc.davos_r
...
cbar = fig.colorbar(cs[3, 1], cax=cax, orientation='horizontal', extend='max',
                    ticks=np.linspace(0, 5, 6, endpoint=True))
cbar.ax.tick_params(labelsize=14)
cbar.ax.set_xlabel('mm hr$^{-1}$', fontsize=14)
...
fig.savefig(plotpath + 'precip_intens_hr.png', dpi=500)
plt.close(fig)
