...
for s in range(len(seasons)):
    ...
    # COSMO 4.4 km
    precip['CPM'][season] = {}
    data = xr.open_dataset(f'{mdpath04}/2001-2005.TOT_PREC.{season}.freq.nc')
    pr = data['TOT_PREC'].values[0, :, :]
    precip['CPM'][season]['precip'] = pr * 4
    precip['CPM']['lon'] = rlon04
    precip['CPM']['lat'] = rlat04
    precip['CPM']['proj'] = rot_pole_crs04
    # COSMO 12 km
    precip['LSM'][season] = {}
    data = xr.open_dataset(f'{mdpath}/2001-2005.TOT_PREC.{season}.freq.nc')
    pr = data['TOT_PREC'].values[0, :, :]
    precip['LSM'][season]['precip'] = pr * 4
    precip['LSM']['lon'] = rlon
    precip['LSM']['lat'] = rlat
    precip['LSM']['proj'] = rot_pole_crs
    # ERA5
    precip['ERA5'][season] = {}
    data = xr.open_dataset(f'{erapath}/freq/2001-2005.day.{season}.freq.nc')
    pr = data['tp'].values[0, :, :]
    precip['ERA5'][season]['precip'] = pr * 4
    precip['ERA5']['lon'] = data['longitude'].values[...]
    precip['ERA5']['lat'] = data['latitude'].values[...]
    precip['ERA5']['proj'] = ccrs.PlateCarree()
    # --
    data = xr.open_dataset(f'{erapath}/remap/2001-2005.day.{season}.freq.remap.04.nc')
    ...
    data = xr.open_dataset(f'{erapath}/remap/2001-2005.day.{season}.freq.remap.nc')
    pr = data['tp'].values[0, :, :] * 4
    ...
    # ERA5
    precip['IMERG'][season] = {}
    data = xr.open_dataset(f'{imergpath}/freq/2001-2005.day.{season}.freq.nc')
    pr = data['precip'].values[0, :, :]
    precip['IMERG'][season]['precip'] = pr * 4
    precip['IMERG']['lon'] = data['lon'].values[...]
    precip['IMERG']['lat'] = data['lat'].values[...]
    precip['IMERG']['proj'] = ccrs.PlateCarree()
    # --
    data = xr.open_dataset(f'{imergpath}/remap/2001-2005.day.{season}.freq.remap.04.nc')
    pr = data['precip'].values[0, :, :] * 4
    precip['CPM'][season]['R']['IMERG'] = \
        ma.corrcoef(ma.masked_invalid(precip['CPM'][season]['precip'][87:-10, 160:-10].flatten()), ma.masked_invalid(pr[87:-10, 160:-10].flatten()))[0, 1]
    precip['CPM'][season]['BIAS']['IMERG'] = np.nanmean(
        precip['CPM'][season]['precip'][87:-10, 160:-10] - pr[87:-10, 160:-10])
    data = xr.open_dataset(f'{imergpath}/remap/2001-2005.day.{season}.freq.remap.nc')
    pr = data['precip'].values[0, :, :] * 4
...
for i in range(len(seasons)):
    ...
    for j in range(2):
        ...
        t1 = axs[i, j + 2].text(0.73, 0.95, "BIAS", fontsize=9, horizontalalignment='center',
                           verticalalignment='center', transform=axs[i, j + 2].transAxes)
        t2 = axs[i, j + 2].text(0.9, 0.95, "R", fontsize=9, horizontalalignment='center',
                           verticalalignment='center', transform=axs[i, j + 2].transAxes)
        t3 = axs[i, j + 2].text(0.46, 0.87, f"CPM - {sim}", fontsize=9, horizontalalignment='center',
                           verticalalignment='center', transform=axs[i, j + 2].transAxes)
        t4 = axs[i, j + 2].text(0.46, 0.79, f"LSM - {sim}", fontsize=9, horizontalalignment='center',
                           verticalalignment='center', transform=axs[i, j + 2].transAxes)
        txt = precip['CPM'][season]['BIAS'][sim]
        t5 = axs[i, j + 2].text(0.73, 0.87, '%0.2f' % txt, fontsize=9, horizontalalignment='center',
                           verticalalignment='center', transform=axs[i, j + 2].transAxes)
        txt = precip['LSM'][season]['BIAS'][sim]
        t6 = axs[i, j + 2].text(0.73, 0.79, '%0.2f' % txt, fontsize=9, horizontalalignment='center',
                           verticalalignment='center', transform=axs[i, j + 2].transAxes)
        txt = precip['CPM'][season]['R'][sim]
        t7 = axs[i, j + 2].text(0.9, 0.87, '%0.2f' % txt, fontsize=9, horizontalalignment='center',
                           verticalalignment='center', transform=axs[i, j + 2].transAxes)
        txt = precip['LSM'][season]['R'][sim]
        t8 = axs[i, j + 2].text(0.9, 0.79, '%0.2f' % txt, fontsize=9, horizontalalignment='center',
                           verticalalignment='center', transform=axs[i, j + 2].transAxes)
...
cbar = fig.colorbar(cs[3, 1], cax=cax, orientation='horizontal',
                    ticks=np.linspace(0, 100, 11, endpoint=True))
...
fig.savefig(plotpath + 'precip_freq_day.png', dpi=500)
plt.close(fig)
