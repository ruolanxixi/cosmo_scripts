...
def pole():
    file = "/scratch/snx3000/rxiang/ruolan/CTRL11/day/01_TOT_PREC.nc"
...
def pole04():
    file = "/scratch/snx3000/rxiang/ruolan/CTRL04/day/01_TOT_PREC.nc"
    ds = xr.open_dataset(f'{file}')
    pole_lat = ds["rotated_pole"].grid_north_pole_latitude
    pole_lon = ds["rotated_pole"].grid_north_pole_longitude
    lat = ds["lat"].values
...
# --- edit here
obspath = "/project/pr133/rxiang/data/cosmo/EAS04_ctrl/indices/hr"
# ---
cpmpath = "/project/pr133/rxiang/data/cosmo/EAS04_ctrl/indices/hr"
lsmpath = "/project/pr133/rxiang/data/cosmo/EAS11_ctrl/indices/hr"
paths = [obspath, cpmpath, lsmpath]
data = {}
vars = ['wet_day_freq', 'intensity', 'perc_99.00', 'perc_99.90']
...
level1 = MaxNLocator(nbins=20).tick_values(0, 0.5)
cmap1 = prcp(20)
norm1 = BoundaryNorm(level1, ncolors=cmap1.N, clip=True)
tick1 = np.linspace(0, 0.5, 6, endpoint=True)

level2 = MaxNLocator(nbins=30).tick_values(0, 5)
cmap2 = prcp(30)
norm2 = BoundaryNorm(level2, ncolors=cmap2.N, clip=True)
tick2 = np.linspace(0, 5, 6, endpoint=True)

level3 = MaxNLocator(nbins=15).tick_values(0, 15)
cmap3 = prcp(15)
norm3 = BoundaryNorm(level3, ncolors=cmap3.N, clip=True)
tick3 = np.linspace(0, 15, 6, endpoint=True)

level4 = MaxNLocator(nbins=25).tick_values(0, 50)
cmap4 = prcp(25)
norm4 = BoundaryNorm(level4, ncolors=cmap4.N, clip=True)
tick4 = np.linspace(0, 50, 6, endpoint=True)
...
extends = ['max', 'max', 'max', 'max']
for i in range(nrow):
    ext = extends[i]
    tick = ticks[i]
    cax = fig.add_axes(
        [axs[i, 2].get_position().x1 + 0.01, axs[i, 2].get_position().y0, 0.015, axs[i, 2].get_position().height])
    cbar = fig.colorbar(cs[i, 2], cax=cax, orientation='vertical', extend=ext, ticks=tick)
