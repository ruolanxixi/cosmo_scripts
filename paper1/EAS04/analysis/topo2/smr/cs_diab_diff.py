...
class Plot_Cross(UserDict):
    ...
    def add_terrain(self, xyline, level=57):
        ...
        self.ax.plot(np.arange(terrain1.shape[0]), terrain1 / 1000, color='red',
                     linewidth=0.7)
    ...
    def add_profile(self, var, varname, w_factor=None, colorbar=True):
        ...
        # levels = MaxNLocator(nbins=14).tick_values(-7, 7)
        norm = colors.TwoSlopeNorm(vmin=-7., vcenter=0., vmax=7)
...
if __name__ == '__main__':
    data_d1 = xr.open_dataset(
        '/project/pr133/rxiang/data/cosmo/EAS04_topo2/szn/DIAB_SUM/2001-2005.DIAB_SUM.JJA.nc')
    ...
    data.save_fig('/project/pr133/rxiang/figure/paper1/results/TENV/diab_diff_sm', format='png', dpi=550,
                   transparent=True)
    plt.show()
    # data.save_fig()
