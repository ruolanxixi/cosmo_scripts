...
from plotcosmomap import plotcosmo04, pole04
...
for s in range(len(sims)):
    sim = sims[s]
    data[sim] = {}
    ds = xr.open_dataset(f'{path}' + f'EAS04_{sim}/day/TOT_PREC/' + f'01-05.TOT_PREC.nc')
    cpm = ds['TOT_PREC'].values[...]
    freq = (cpm[121:274] > 1).sum(axis=0)/len(cpm[121:274, 0, 0])*100
    data[sim]['freq'] = freq
    mask = cpm > 1
    intens = np.nanmean(cpm * mask, axis=0)
    data[sim]['intens'] = intens
...
hsurf_diff = ndimage.gaussian_filter(hsurf_topo2 - hsurf_ctrl, sigma=3, order=0)
...
levels2 = MaxNLocator(nbins=15).tick_values(-40, 40)
...
for j in range(ncol):
    ...
    cmap = cmaps[j]
    norm = norms[j]
    cs[0, j] = axs[0, j].pcolormesh(rlon, rlat, data[sim]['freq'], cmap=cmap, norm=norm, shading="auto")
# --
cax = fig.add_axes([axs[0, 1].get_position().x1+0.01, axs[0, 1].get_position().y0, 0.015, axs[0, 1].get_position().height])
cbar = fig.colorbar(cs[0, 1], cax=cax, orientation='vertical', ticks=[0, 20, 40, 60, 80, 100])
...
levels2 = MaxNLocator(nbins=21).tick_values(-5, 5)
...
for j in range(ncol):
    ...
    norm = norms[j]
    scale = scales[j]
    cs[1, j] = axs[1, j].pcolormesh(rlon, rlat, data[sim]['intens'], cmap=cmap, norm=norm, shading="auto")
