...
class Plot_Cross(UserDict):
    ...
    def add_terrain(self, xyline, level=57):
        ...
        self.ax.plot(np.arange(terrain1.shape[0]), terrain1 / 1000, color='black',
                     linewidth=0.01)
        self.ax.fill_between(np.arange(terrain1.shape[0]), terrain1 / 1000, color='black')

        terrain2 = wrf.interp2dxy(hsurf2, xyline)[1]
        self.ax.plot(np.arange(terrain2.shape[0]), terrain2 / 1000, color='white', linewidth=0.7)
    ...
    def add_profile(self, var, varname, w_factor=None, colorbar=True):
        ...
        levels = np.linspace(-2, 1, 20, endpoint=True)
        ...
        # self.ax.set_xticks(np.linspace(0, 240, 7, endpoint=True))
        ...
        cbar.set_ticks([-2, -1.5, -1, -0.5, 0, 0.5, 1])
...
if __name__ == '__main__':
    ...
    data.save_fig('/project/pr133/rxiang/figure/paper1/results/TENV/qv_diff_sm.png', format='png', dpi=550,
                   transparent=True)
    plt.show()
    # data.save_fig()
