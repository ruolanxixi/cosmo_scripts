...
era11data = xr.open_dataset(f'{erapath}{filename}')['tp'].values[0, :, :] * 1000
filename = f'cru.{year}.05.remap.11.nc'
cru11data = xr.open_dataset(f'{crupath}{filename}')['pre'].values[0, :, :]
filename = f'{year}.remap.11.nc4'
imerg11data = xr.open_dataset(f'{imergpath}{filename}')['pr'].values[0, :, :]
...
for i in range(nrow-1):
    cs[i % nrow, i // nrow] = axs[i % nrow, i // nrow].pcolormesh(rlon, rlat, mddata[i], cmap=cmc.davos_r, clim=(0, 30), shading="auto")
    ax = plotcosmo04(axs[i % nrow, i // nrow])

cs[4, 0] = axs[4, 0].pcolormesh(rlon11, rlat11, md11data, cmap=cmc.davos_r, clim=(0, 30), shading="auto")
